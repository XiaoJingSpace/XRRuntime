    ...
    {    
        ...
        if( byteBuffer != 0 )
        {
            ...
            if( bufferSize <= memorySize )
            {
                switch(what)
                {
                    case sxrControllerQueryType::kControllerQueryBatteryRemaining:
                    {
                        ::memcpy(memory, bufferPtr, 4);
                        bytesWritten = 4;
                    }
                    break;
                    case sxrControllerQueryType::kControllerQueryControllerCaps:
                    {
                        ::memcpy(memory, bufferPtr, 148);
                        bytesWritten = 148;
                    }
                    break;
                    case sxrControllerQueryType::kControllerQueryControllerMeta:
                    {
                        ::memcpy(memory, bufferPtr, bufferSize);
...
//-----------------------------------------------------------------------------
bool ControllerManager::ControllerGetState(int handle, sxrControllerState* data)
{
    bool result = false;

    if( IsValid(handle) )
    {
        RingBuffer<sxrControllerState>* ringBuffer = listOfControllers[handle];
        if( ringBuffer != 0 )
        {
            sxrControllerState* currentData = ringBuffer->get();
            if( currentData != 0x00 )
            {
                ::memcpy(data, currentData, sizeof(sxrControllerState));
                result = true;
            }
        }
    }
...
//-----------------------------------------------------------------------------
void ControllerManager::CachePtr(int handle, RingBuffer<sxrControllerState>* ptr)
{
    if( IsValid(handle) )
    {
...
void ControllerManager::OnControllerCallback(int what, long arg1, long arg2, jobject dataBundle)
{
    ...
    switch(what)
    {
        case CONTROLLER_MEMORY_ALLOCATED:
            CachePtr(arg1, (RingBuffer<sxrControllerState>*)(arg2));
            break;
