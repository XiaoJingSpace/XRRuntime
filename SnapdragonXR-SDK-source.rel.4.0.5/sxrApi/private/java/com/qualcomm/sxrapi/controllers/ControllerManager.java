...
import com.qualcomm.sxrapi.SvrServiceClient;
    ...
    public ByteBuffer Query(int handle, int what)
    {
        ...
        if( controllerContext != null ) {
            switch (what) {
                ...
                break;
                case SvrControllerApi.svrControllerQueryType_kControllerQueryMeta:
                {
                    serializedData = ByteBuffer.allocateDirect(2048);
                    String fromService = controllerContext.QueryString(SvrControllerApi.svrControllerQueryType_kControllerQueryMeta);
                    serializedData.order(ByteOrder.LITTLE_ENDIAN);
                    serializedData.position(0);
                    byte[] deviceMetaBytes = fromService.getBytes();
                    serializedData.put(deviceMetaBytes); 
                }
                break;
            }
        }
