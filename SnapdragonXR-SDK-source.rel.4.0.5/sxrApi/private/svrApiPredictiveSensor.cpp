...
VAR(float, gSensorHeadOffsetX, 0.0f, kVariableNonpersistent);       //Adjustment for device physical distance from head (meters)
VAR(float, gSensorHeadOffsetY, 0.0f, kVariableNonpersistent);       //Adjustment for device physical distance from head (meters)
VAR(float, gSensorHeadOffsetZ, 0.0762, kVariableNonpersistent);     //Adjustment for device physical distance from head (meters)

VAR(float, gMinPoseQuality, 0.7, kVariableNonpersistent);           // Minimum pose quality (In range [0.0, 1.0]) allowed

VAR(float, gIMUSensorPoseDelay, 0.0, kVariableNonpersistent);       // Base IMU delay (milliseconds).  How old is pose before it is recorded?

VAR(float, gMaxPredictedTime, 75.0, kVariableNonpersistent);        // Max allowable predicted pose time (milliseconds)
VAR(bool, gLogMaxPredictedTime, true, kVariableNonpersistent);     //Enable/disable logging when max predicted time is enforced


// Logging Options
VAR(bool, gLogRawSensorData, false, kVariableNonpersistent);            //Enable/disable logging of sensor values directly from the service

// Debug Fixed Rotation
VAR(bool, gUseFixedRotation, false, kVariableNonpersistent);            //Enable/disable setting of sensor rotation value directly
VAR(float, gFixedRotationQuatX, 0.0f, kVariableNonpersistent);
VAR(float, gFixedRotationQuatY, 0.0f, kVariableNonpersistent);
VAR(float, gFixedRotationQuatZ, 0.0f, kVariableNonpersistent);
VAR(float, gFixedRotationQuatW, 1.0f, kVariableNonpersistent);

// Fixed rotation speed (Euler Angles) subject to gUseFixedRotation
VAR(float, gFixedRotationSpeedX, 0.0f, kVariableNonpersistent);
VAR(float, gFixedRotationSpeedY, 0.0f, kVariableNonpersistent);
VAR(float, gFixedRotationSpeedZ, 0.0f, kVariableNonpersistent);

// Fixed rotation bounce angle (Euler Angles) subject to gUseFixedRotation
VAR(float, gFixedRotationBounceX, 0.0f, kVariableNonpersistent);
VAR(float, gFixedRotationBounceY, 0.0f, kVariableNonpersistent);
VAR(float, gFixedRotationBounceZ, 0.0f, kVariableNonpersistent);

// Debug Fixed Position
VAR(bool, gUseFixedPosition, false, kVariableNonpersistent);            //Enable/disable setting of sensor position value directly
VAR(float, gFixedPositionX, 0.0f, kVariableNonpersistent);
VAR(float, gFixedPositionY, 0.0f, kVariableNonpersistent);
VAR(float, gFixedPositionZ, 0.0f, kVariableNonpersistent);

VAR(float, gFixedPositionEndX, 0.0f, kVariableNonpersistent);
VAR(float, gFixedPositionEndY, 0.0f, kVariableNonpersistent);
VAR(float, gFixedPositionEndZ, 0.0f, kVariableNonpersistent);
VAR(float, gFixedPositionSpeed, 0.0f, kVariableNonpersistent);
...
int gCurrentFixedRotationDirectionX = 1;
int gCurrentFixedRotationDirectionY = 1;
int gCurrentFixedRotationDirectionZ = 1;
...
void _predict_translation(const float* translation_s, const float* translation_b, float* translation, float delay_ms)
{

    if (NULL == translation_s || NULL == translation_b) {
        return;
    }

    const float f1 = 1.021f;

    float delay_s = delay_ms / 1000.f;


    for (int i = 0; i < 3; ++i) {
        translation[i] = translation_s[i] + translation_b[i] * delay_s / f1;
    }
}
...
void L_TrackingToReturn(sxrHeadPoseState& poseState, const float* gyro_data_s, const float* gyro_data_b, const float* gyro_data_bdt, const float* gyro_data_bdt2, float fw_prediction_delay, const float* rotation, const float* translation, const float* prediction_coff_ts, const float* prediction_coff_tb)
{
    if(gLogRawSensorData)
    {
        ...
        LOGI("Raw Sensor Position: (%0.4f, %0.4f, %0.4f)", translation[0], translation[1], translation[2]);
        LOGI("Raw Sensor Rotation: (%0.4f, %0.4f, %0.4f, %0.4f) => Euler(%0.4f, %0.4f, %0.4f)", rotation[0], rotation[1], rotation[2], rotation[3], glm::degrees(euler.x), glm::degrees(euler.y), glm::degrees(euler.z));
    }
    ...
    if (fw_prediction_delay != 0.0f)
    {
        ...
        if (gLogRawSensorData)
        {
            LOGI("    Prediction time adjusted by pose age (%0.4f) and IMU Delay (%0.4f): %0.4f -> %0.4f", poseAge, gIMUSensorPoseDelay, fw_prediction_delay, fw_prediction_delay + poseAge + gIMUSensorPoseDelay);
        }
    ...
    */
    if (fw_prediction_delay != 0.0f && gyro_data_s != NULL && gyro_data_b != NULL && gyro_data_bdt != NULL && gyro_data_bdt2 != NULL)
    {
        if (fw_prediction_delay > gMaxPredictedTime)
        {
            if (gLogMaxPredictedTime)
            {
                LOGI("    Maximum Prediction time allowed is %0.4f ms. Requested time of %0.4f ms will be clamped!", gMaxPredictedTime, fw_prediction_delay);
            }
        ...
        if (gLogRawSensorData)
        {
            LOGI("    Prediction of %0.4f ms from pose time stamp with Coefficients = (%0.4f, %0.4f, %0.4f, %0.4f)", fw_prediction_delay, *gyro_data_s, *gyro_data_b, *gyro_data_bdt, *gyro_data_bdt2);
        }

        _predict_motion(gyro_data, rotation, quatOut, fw_prediction_delay, gyro_data_s, gyro_data_b, gyro_data_bdt, gyro_data_bdt2);
    ...
    if(gLogRawSensorData)
    {
        glm::fquat tempQuat(poseState.pose.rotation.w, poseState.pose.rotation.x, poseState.pose.rotation.y, poseState.pose.rotation.z);
        glm::vec3 euler = eulerAngles(tempQuat);
        LOGI("    Predicted Sensor Quat: (%0.4f, %0.4f, %0.4f, %0.4f) => Euler(%0.4f, %0.4f, %0.4f)", poseState.pose.rotation.x, poseState.pose.rotation.y, poseState.pose.rotation.z, poseState.pose.rotation.w, glm::degrees(euler.x), glm::degrees(euler.y), glm::degrees(euler.z));
    }
    ...
    if(gLogRawSensorData && (gSensorOrientationCorrectX != 0.0 || gSensorOrientationCorrectY != 0.0 || gSensorOrientationCorrectZ != 0.0))
    {
        glm::fquat tempQuat(poseState.pose.rotation.w, poseState.pose.rotation.x, poseState.pose.rotation.y, poseState.pose.rotation.z);
        glm::vec3 euler = eulerAngles(tempQuat);
        LOGI("    Adjusted for Sensor Orientation: (%0.2f, %0.2f, %0.2f, %0.2f) => Euler(%0.2f, %0.2f, %0.2f)", poseState.pose.rotation.x, poseState.pose.rotation.y, poseState.pose.rotation.z, poseState.pose.rotation.w, glm::degrees(euler.x), glm::degrees(euler.y), glm::degrees(euler.z));
    }
    ...
    float predicted_translation[3] = { translation[0], translation[1], translation[2] };
    if (fw_prediction_delay != 0.0f && NULL != prediction_coff_ts && NULL != prediction_coff_tb) {
        _predict_translation(prediction_coff_ts, prediction_coff_tb, predicted_translation, fw_prediction_delay);
    }

    if (gLogRawSensorData)
    {
        LOGI("    Adjusted for Sensor Translation: (%0.4f, %0.4f, %0.4f)", predicted_translation[0], predicted_translation[1], predicted_translation[2]);
    }
    ...
    input[1] = r[1];
    input[2] = r[2];
    input[3] = -predicted_translation[0];

    input[4] = r[3];
    input[5] = r[4];
    input[6] = r[5];
    input[7] = -predicted_translation[1];

    input[8] = r[6];
    input[9] = r[7];
    input[10] = r[8];
    input[11] = -predicted_translation[2];
    ...
    if (gAppContext->currentTrackingMode & kTrackingPosition)
    {
        ...
        glm::vec3 baseOffset = glm::vec3(gSensorHeadOffsetX, gSensorHeadOffsetY, gSensorHeadOffsetZ);
        glm::vec3 rotOffset = baseOffset * TempQuat;
        glm::vec3 diffOffset = baseOffset - rotOffset;

        if (gLogRawSensorData && (gSensorHeadOffsetX != 0.0 || gSensorHeadOffsetY != 0.0 || gSensorHeadOffsetZ != 0.0))
        {
            LOGI("    Adjustment to position (%0.3f, %0.3f, %0.3f) based on head offset: (%0.3f, %0.3f, %0.3f)", poseState.pose.position.x, poseState.pose.position.y, poseState.pose.position.z, diffOffset.x, diffOffset.y, diffOffset.z);
        }
    ...
    if(gLogRawSensorData)
    {
        glm::fquat tempQuat(poseState.pose.rotation.w, poseState.pose.rotation.x, poseState.pose.rotation.y, poseState.pose.rotation.z);
        glm::vec3 euler = eulerAngles(tempQuat);
        LOGI("    Final Returned Orientation: (%0.4f, %0.4f, %0.4f, %0.4f) => Euler(%0.4f, %0.4f, %0.4f)", poseState.pose.rotation.x, poseState.pose.rotation.y, poseState.pose.rotation.z, poseState.pose.rotation.w, glm::degrees(euler.x), glm::degrees(euler.y), glm::degrees(euler.z));
    }
}

void L_CheckPoseQuality(sxrHeadPoseState& poseState, qvrservice_head_tracking_data_t *t, bool shouldPostEvents)
{
    if (gAppContext == NULL)
    {
    ...
    if ((gAppContext->currentTrackingMode & kTrackingPosition) && t->pose_quality < gMinPoseQuality)
    {
        if (gLogRawSensorData)
        {
            LOGE("Pose quality is below allowed value: %0.2f < %0.2f", t->pose_quality, gMinPoseQuality);
        }
...
//--------------------------------------------------------------------------------------------------------
{
    ...
    if (qvrReturn != QVR_SUCCESS)
    {
        ...
        default:
            if (diffTimeMs > spamTime)
            {
                LOGE("Error from QVRServiceClient_GetHeadTrackingData: Unknown = %d", qvrReturn);
                gLastHeadTrackingFailTime = timeNowNano;
            }
            return SXR_ERROR_UNKNOWN;
        }
    ...
    if (t->rotation[0] == 0.0f && t->rotation[1] == 0.0f && t->rotation[2] == 0.0f && t->rotation[3] == 0.0f)
    {
    ...
    *pSampleTimeStamp = t->ts;
    ...
    L_TrackingToReturn(poseState, t->prediction_coff_s, t->prediction_coff_b, t->prediction_coff_bdt, t->prediction_coff_bdt2, fw_prediction_delay, rotation, translation, t->prediction_coff_ts, t->prediction_coff_tb);
    ...
    if (gUseFixedRotation)
    {
        ...
        if (gFixedRotationSpeedX != 0.0f || gFixedRotationSpeedY != 0.0f || gFixedRotationSpeedZ != 0.0f)
        {
            // How much time has elapsed since last time
            uint64_t timeNowNano = Svr::GetTimeNano();
            ...
            gTotalFixedRotationX += gCurrentFixedRotationDirectionX * diffTimeMs * gFixedRotationSpeedX / 1000.0f;
            gTotalFixedRotationY += gCurrentFixedRotationDirectionY * diffTimeMs * gFixedRotationSpeedY / 1000.0f;
            gTotalFixedRotationZ += gCurrentFixedRotationDirectionZ * diffTimeMs * gFixedRotationSpeedZ / 1000.0f;

            if (gFixedRotationBounceX > 0.0f)
            {
                if (gTotalFixedRotationX > gFixedRotationBounceX)
                    gCurrentFixedRotationDirectionX = -1;
                else if (gTotalFixedRotationX < -gFixedRotationBounceX)
                    gCurrentFixedRotationDirectionX = 1;
            }
            if (gFixedRotationBounceY > 0.0f)
            {
                if (gTotalFixedRotationY > gFixedRotationBounceY)
                    gCurrentFixedRotationDirectionY = -1;
                else if (gTotalFixedRotationY < -gFixedRotationBounceY)
                    gCurrentFixedRotationDirectionY = 1;
            }
            if (gFixedRotationBounceZ > 0.0f)
            {
                if (gTotalFixedRotationZ > gFixedRotationBounceZ)
                    gCurrentFixedRotationDirectionZ = -1;
                else if (gTotalFixedRotationZ < -gFixedRotationBounceZ)
                    gCurrentFixedRotationDirectionZ = 1;
            }
            ...
            // Add in "prediction"
            float tempRotationX = gTotalFixedRotationX + fw_prediction_delay * gFixedRotationSpeedX / 1000.0f;
            float tempRotationY = gTotalFixedRotationY + fw_prediction_delay * gFixedRotationSpeedY / 1000.0f;
            float tempRotationZ = gTotalFixedRotationZ + fw_prediction_delay * gFixedRotationSpeedZ / 1000.0f;

            if (gFixedRotationSpeedX != 0.0f)
                tempQuat = glm::rotate(tempQuat, tempRotationX, glm::vec3(1.0f, 0.0f, 0.0f));

            if (gFixedRotationSpeedY != 0.0f)
                tempQuat = glm::rotate(tempQuat, tempRotationY, glm::vec3(0.0f, 1.0f, 0.0f));

            if (gFixedRotationSpeedZ != 0.0f)
                tempQuat = glm::rotate(tempQuat, tempRotationZ, glm::vec3(0.0f, 0.0f, 1.0f));
        ...
        if (gLogRawSensorData)
        {
            glm::fquat tempQuat(poseState.pose.rotation.w, poseState.pose.rotation.x, poseState.pose.rotation.y, poseState.pose.rotation.z);
            glm::vec3 euler = eulerAngles(tempQuat);
            LOGI("    Config File Fixed Orientation: (%0.2f, %0.2f, %0.2f, %0.2f) => Euler(%0.2f, %0.2f, %0.2f)", poseState.pose.rotation.x, poseState.pose.rotation.y, poseState.pose.rotation.z, poseState.pose.rotation.w, glm::degrees(euler.x), glm::degrees(euler.y), glm::degrees(euler.z));
        }
    ...
    if (gUseFixedPosition)
    {
        ...
        if (gFixedPositionSpeed != 0.0)
        {
            ...
            glm::vec3 direction = glm::vec3(gFixedPositionEndX, gFixedPositionEndY, gFixedPositionEndZ) - glm::vec3(gFixedPositionX, gFixedPositionY, gFixedPositionZ);
            ...
            glm::vec3 totalMovement = glm::vec3(gCurrentFixedPositionX, gCurrentFixedPositionY, gCurrentFixedPositionZ) - glm::vec3(gFixedPositionX, gFixedPositionY, gFixedPositionZ);
            ...
            // Add in "prediction"
            direction = glm::vec3(gFixedPositionEndX, gFixedPositionEndY, gFixedPositionEndZ) - glm::vec3(gFixedPositionX, gFixedPositionY, gFixedPositionZ);
            direction *= (gFixedPositionSpeed * fw_prediction_delay * msToSeconds);
            float tempPositionX = gCurrentFixedPositionX + direction.x;
            float tempPositionY = gCurrentFixedPositionY + direction.y;
            float tempPositionZ = gCurrentFixedPositionZ + direction.z;

            poseState.pose.position.x = -tempPositionX;
            poseState.pose.position.y = -tempPositionY;
            poseState.pose.position.z = -tempPositionZ;
        }   // Fixed position has a speed

        if (gLogRawSensorData)
        {
            LOGI("    Config File Fixed Position: (%0.3f, %0.3f, %0.3f)", -poseState.pose.position.x, -poseState.pose.position.y, -poseState.pose.position.z);
        }
    ...
    if (t->flags_3dof_mag_used && !t->flags_3dof_mag_calibrated)
    {
        ...
        if (((timeNow - lastMagEventTime) * NANOSECONDS_TO_MILLISECONDS) > 5000.0f)
        {
            ...
            if (gAppContext != NULL && gAppContext->modeContext != NULL && gAppContext->modeContext->eventManager != NULL)
            {
                svrEventQueue* pQueue = gAppContext->modeContext->eventManager->AcquireEventQueue();
                if (pQueue != NULL)
                {
...
//--------------------------------------------------------------------------------------------------------
{
    ...
    int32_t qvrReturn;
    qvrservice_head_tracking_data_t *t = NULL;
    qvrReturn = QVRServiceClient_GetHistoricalHeadTrackingData(gAppContext->qvrHelper, &t, timestampNs);
    if(qvrReturn != QVR_SUCCESS)
    {
        switch(qvrReturn)
        {
        ...
        default:
            LOGE("Error from QVRServiceClient_GetHistoricalHeadTrackingData: Unknown = %d", qvrReturn);
            break;
        }
    ...
    const float* rotation= t->rotation;
    const float* translation = t->translation;
    L_TrackingToReturn(poseState, NULL, NULL, NULL, NULL, 0.0f, rotation, translation, NULL, NULL); // Note NULL and 0.0f since not doing any prediction on historic data :)
