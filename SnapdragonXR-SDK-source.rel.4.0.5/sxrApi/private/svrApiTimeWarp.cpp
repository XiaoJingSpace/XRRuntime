...
#define RENDER_ACTUAL_MESH
#ifdef ENABLE_CAMERA
#include "svrCameraManager.h"
#endif

#include "sxrDistortionMesh.h"
SxrDistortionMesh gSxrDistortionMesh;
...
VAR(int, gTimeWarpThreadCore, 2, kVariableNonpersistent);               //Core affinity for time warp thread (-1=system defined), ignored if the QVR Perf module is active.
...
VAR(bool, gEnableWarpThreadFifo, true, kVariableNonpersistent);         //Enable/disable setting SCHED_FIFO scheduling policy on the warp thread (must be true if gBusyWait is false to avoid tearing), ignored if the QVR Perf module is active.
VAR(int, gRecenterFrames, 0, kVariableNonpersistent);                   //Number of frames to disable reprojection after sensors are recentered
VAR(int, gSensorInitializeFrames, 0, kVariableNonpersistent);           //Number of frames to disable display after sensors are started
VAR(int, gTimeWarpMinLoopTime, 0, kVariableNonpersistent);              //For testing: minimum time (milliseconds) for a timewarp loop.  Can be used to simulate timewarp not able to keep up with render. Set to 0 to disable test
VAR(float, gTimeWarpWaitBias, 0.0f, kVariableNonpersistent);            //Bias applied to the time we wait to kick off warping at mid frame
VAR(float, gTimeWarpWaitFramePercentage, 0.5f, kVariableNonpersistent); //Percent of the frame time the display buffer has transfered half the data (default .5)
VAR(bool, gTimeWarpDelayBetweenEyes, true, kVariableNonpersistent);     //Second eye is rendered at apporpriate vsync time. Otherwise, both eyes rendered back to back
VAR(bool, gSwapEyeBuffers, false, kVariableNonpersistent);              //Had a device that swapped DSI 0/1 so left eye went to right eye and right to left

VAR(float, gTimeWarpClearColorR, 0.0f, kVariableNonpersistent);         //Fill color for TimeWarp clear (if gTimeWarpClearBuffer enabled)
VAR(float, gTimeWarpClearColorG, 0.0f, kVariableNonpersistent);         //Fill color for TimeWarp clear (if gTimeWarpClearBuffer enabled)
VAR(float, gTimeWarpClearColorB, 0.0f, kVariableNonpersistent);         //Fill color for TimeWarp clear (if gTimeWarpClearBuffer enabled)
VAR(float, gTimeWarpClearColorA, 1.0f, kVariableNonpersistent);         //Fill color for TimeWarp clear (if gTimeWarpClearBuffer enabled)

VAR(bool, gSupportColorPlaneWarp, false, kVariableNonpersistent);       // Some displays support different color planes.
VAR(float, gColorPlaneTimeOffsetR, 0.0f, kVariableNonpersistent);       // When warping, each color plane has a different time (milliseconds) to be warped
VAR(float, gColorPlaneTimeOffsetG, 0.0f, kVariableNonpersistent);       // When warping, each color plane has a different time (milliseconds) to be warped
VAR(float, gColorPlaneTimeOffsetB, 0.0f, kVariableNonpersistent);       // When warping, each color plane has a different time (milliseconds) to be warped

VAR(int, gTimeWarpEnabledEyeMask, 3, kVariableNonpersistent);           // sxrEyeMask Bit Field: 1 = Left; 2 = Right; 3 = Both

VAR(int, gForceColorSpace, -1.0f, kVariableNonpersistent);              //Force warp colorspace -1 = app specified, 0 = linear, 1 = sRGB

VAR(bool, gApplyDisplaySkew, false, kVariableNonpersistent);            //Apply a skew during warp to counteract the impact of display rates
VAR(bool, gApplyDisplaySquash, false, kVariableNonpersistent);          //Apply a skew during warp to counteract the impact of display rates
VAR(bool, gApplyDisplayBias, false, kVariableNonpersistent);            //Apply a bias during warp to counteract the impact of display rates

VAR(float, gSkewScaleFactor, 1.0f, kVariableNonpersistent);             //Scale factor to apply to the display skewing
VAR(float, gSquashScaleFactor, 1.0f, kVariableNonpersistent);           //Scale factor to apply to the display squash
VAR(float, gBiasScaleFactor, 1.0f, kVariableNonpersistent);             //Bias to shift - fraction of a frame time - default value is 1 frame, so 2 = 2 frames later, 0 is no bias

// Compositing value
VAR(float, gCompositeRadius, 0.25f, kVariableNonpersistent);            //Any layer quad outside this range will be composited


// Warp Mesh
VAR(int, gWarpMeshType, 0, kVariableNonpersistent);                     //Warp mesh type: 0 = Columns (Left To Right); 1 = Columns (Right To Left); 2 = Rows (Top To Bottom); 3 = Rows (Bottom To Top)
VAR(int, gWarpMeshRows, 50, kVariableNonpersistent);                    //Number of rows in the warp mesh grid
VAR(int, gWarpMeshCols, 50, kVariableNonpersistent);                    //Number of columns in the warp mesh grid
VAR(int, gLayerMeshRows, 8, kVariableNonpersistent);                    //Number of rows in the layer mesh grid
VAR(int, gLayerMeshCols, 8, kVariableNonpersistent);                    //Number of columns in the layer mesh grid

VAR(float, gWarpEqrMeshRadius, 2.0, kVariableNonpersistent);            //Radius of warp mesh used for EquiRectangular display (Affects curvature)
VAR(float, gWarpEqrMeshScale, 0.9, kVariableNonpersistent);             //Amount of screen covered by the EquiRectangular warp mesh


VAR(float, gWarpMeshMinX, -1.0f, kVariableNonpersistent);               //Screen space coordinate of minimum X-Value
VAR(float, gWarpMeshMaxX, 1.0f, kVariableNonpersistent);                //Screen space coordinate of maximum X-Value
VAR(float, gWarpMeshMinY, -1.0f, kVariableNonpersistent);               //Screen space coordinate of minimum Y-Value
VAR(float, gWarpMeshMaxY, 1.0f, kVariableNonpersistent);                //Screen space coordinate of maximum Y-Value

VAR(float, gMeshOffsetLeftX, 0.0f, kVariableNonpersistent);             // Screen space adjustment for left mesh to center under lens
VAR(float, gMeshOffsetLeftY, 0.0f, kVariableNonpersistent);             // Screen space adjustment for left mesh to center under lens
VAR(float, gMeshOffsetRightX, 0.0f, kVariableNonpersistent);            // Screen space adjustment for right mesh to center under lens
VAR(float, gMeshOffsetRightY, 0.0f, kVariableNonpersistent);            // Screen space adjustment for right mesh to center under lens

// Vignette
VAR(bool, gMeshVignette, false, kVariableNonpersistent);                //Enable/Disable vignette feature on the warp mesh
VAR(float, gVignetteRadius, 0.7f, kVariableNonpersistent);              //Start radius of vignette feature. Completes at gMeshDiscardUV applied to lens polynomial

// Clamp to Border
VAR(bool, gClampBorderEnabled, true, kVariableNonpersistent);           //Enable/disable clamp to border color on eye buffers
VAR(float, gClampBorderColorR, 0.0f, kVariableNonpersistent);           //Fill color for clamp to border (if enabled)
VAR(float, gClampBorderColorG, 0.0f, kVariableNonpersistent);           //Fill color for clamp to border (if enabled)
VAR(float, gClampBorderColorB, 0.0f, kVariableNonpersistent);           //Fill color for clamp to border (if enabled)
VAR(float, gClampBorderColorA, 1.0f, kVariableNonpersistent);           //Fill color for clamp to border (if enabled)

VAR(bool, gClampBorderOverlayEnabled, true, kVariableNonpersistent);    //Enable/disable clamp to border color on overlay buffers
VAR(float, gClampBorderOverlayColorR, 0.0f, kVariableNonpersistent);    //Fill color for clamp to border (if enabled)
VAR(float, gClampBorderOverlayColorG, 0.0f, kVariableNonpersistent);    //Fill color for clamp to border (if enabled)
VAR(float, gClampBorderOverlayColorB, 0.0f, kVariableNonpersistent);    //Fill color for clamp to border (if enabled)
VAR(float, gClampBorderOverlayColorA, 1.0f, kVariableNonpersistent);    //Fill color for clamp to border (if enabled)

// Occluder Mesh
VAR(bool, gOcclusionMeshEnabled, false, kVariableNonpersistent);          //Enable/disable occlude mesh 3dr
VAR(float, gOcclusionMeshColorR, 0.0f, kVariableNonpersistent);           //Fill color for occlude mesh (if enabled)
VAR(float, gOcclusionMeshColorG, 0.0f, kVariableNonpersistent);           //Fill color for occlude mesh (if enabled)
VAR(float, gOcclusionMeshColorB, 0.0f, kVariableNonpersistent);           //Fill color for occlude mesh (if enabled)
VAR(float, gOcclusionMeshColorA, 0.0f, kVariableNonpersistent);           //Fill alpha for occlude mesh (if enabled)
...
// Boundary System
VAR(bool, gEnableBoundarySystem, false, kVariableNonpersistent);        //Enable/disable Boundary display
VAR(bool, gForceDisplayBoundarySystem, false, kVariableNonpersistent);  //Forces the display of the Boundary display
VAR(bool, gLogBoundarySystem, false, kVariableNonpersistent);           //Enable/disable loging of Boundary system

VAR(bool, gEnableBoundaryMesh, false, kVariableNonpersistent);          //Enable/disable Boundary feature using mesh
VAR(int, gBoundaryWarpMeshRows, 2, kVariableNonpersistent);             //Number of rows in the Boundary mesh
VAR(int, gBoundaryWarpMeshCols, 2, kVariableNonpersistent);             //Number of cols in the Boundary mesh
VAR(float, gBoundaryZoneMinX, -4.0, kVariableNonpersistent);            //Boundary zone size (Real world coordinates in meters)
VAR(float, gBoundaryZoneMaxX, 4.0, kVariableNonpersistent);             //Boundary zone size (Real world coordinates in meters)
VAR(float, gBoundaryZoneMinY, -4.0, kVariableNonpersistent);            //Boundary zone size (Real world coordinates in meters)
VAR(float, gBoundaryZoneMaxY, 4.0, kVariableNonpersistent);             //Boundary zone size (Real world coordinates in meters)
VAR(float, gBoundaryZoneMinZ, -4.0, kVariableNonpersistent);            //Boundary zone size (Real world coordinates in meters)
VAR(float, gBoundaryZoneMaxZ, 4.0, kVariableNonpersistent);             //Boundary zone size (Real world coordinates in meters)
VAR(float, gBoundaryUVScale, 10.0, kVariableNonpersistent);             //Boundary zone texture repeat scale (grid spacing)
VAR(float, gBoundaryGridWidth, 0.1, kVariableNonpersistent);            //Boundary zone grid line width (fractional part of result of gBoundaryUVScale)
VAR(float, gBoundaryVisabilityRadius, 0.5, kVariableNonpersistent);     //Distance from Boundary zone boundaries when the Boundary mesh starts to be visible
VAR(float, gBoundaryVisabilityScale, 1.5, kVariableNonpersistent);      //Scale for how quickly the Boundary mesh fades in to full opacity
VAR(float, gBoundaryMeshColorR, 1.0, kVariableNonpersistent);           //Boundary mesh color
VAR(float, gBoundaryMeshColorG, 1.0, kVariableNonpersistent);           //Boundary mesh color
VAR(float, gBoundaryMeshColorB, 1.0, kVariableNonpersistent);           //Boundary mesh color
VAR(float, gBoundaryMeshColorA, 1.0, kVariableNonpersistent);           //Boundary mesh color


// Lens Parameters
VAR(float, gLensOffsetLeftX, 0.0f, kVariableNonpersistent);             //Horizontal lens offset left
VAR(float, gLensOffsetLeftY, 0.0f, kVariableNonpersistent);             //Vertical lens offset left
VAR(float, gLensOffsetRightX, 0.0f, kVariableNonpersistent);            //Horizontal lens offset right
VAR(float, gLensOffsetRightY, 0.0f, kVariableNonpersistent);            //Vertical lens offset right

VAR(float, gLensScale, 1.0f, kVariableNonpersistent);                   //Uniform lens scale
VAR(float, gLensScaleX, 0.0f, kVariableNonpersistent);                  //Uniform lens scale width (0 means undefined)
VAR(float, gLensScaleY, 0.0f, kVariableNonpersistent);                  //Uniform lens scale height (0 means undefined)

VAR(bool, gLensPolyUseXYZ, false, kVariableNonpersistent);
...
// Lens XYZ Distortion Polynomial: K0 + K1*r + K2*r^2 + K3*r^3 + K4*r^4 + K5*r^5 + K6*r^6
// Distort mesh vert XYZ using this polynomial. Used for tracking camera lens mesh.
// These numbers are for the Trinity.  For SVR845 they will be different.
VAR(float, gLensXYZPolyK0, 1.0f, kVariableNonpersistent);
VAR(float, gLensXYZPolyK1, 0.0f, kVariableNonpersistent);
VAR(float, gLensXYZPolyK2, -0.3940f, kVariableNonpersistent);
VAR(float, gLensXYZPolyK3, 0.0f, kVariableNonpersistent);
VAR(float, gLensXYZPolyK4, 0.1234, kVariableNonpersistent);
VAR(float, gLensXYZPolyK5, 0.0f, kVariableNonpersistent);
VAR(float, gLensXYZPolyK6, -0.0211, kVariableNonpersistent);
...
VAR(float, gChromaticPixelBorder, 0.0f, kVariableNonpersistent);

// Warp Mesh discard UV value
VAR(float, gMeshDiscardUV, 1.0f, kVariableNonpersistent);               //After lens polynomial applied, anything greater than this value will be discarded.
...
VAR(bool, gLogVSyncData, false, kVariableNonpersistent);                //Enables output of LogCat messages when VSync data is updated
VAR(bool, gLogPrediction, false, kVariableNonpersistent);
VAR(bool, gLogDroppedFrames, false, kVariableNonpersistent);            //If next frame warped is not one more than last frame (Application may break this if it assigns wrong frameIndx)

VAR(bool, gLogShaderUniforms, false, kVariableNonpersistent);
VAR(bool, gLogEyeRender, false, kVariableNonpersistent);


// Display lens correction
VAR(float, gScreenWidthMM, 125.4400f, kVariableNonpersistent); // SVR845
//VAR(float, gScreenWidthMM, 103.68f, kVariableNonpersistent); // Trinity/Morpheus

// Distortion Mesh Data
VAR(bool, gUseDistortionMeshData, false, kVariableNonpersistent);
...
EXTERN_VAR(bool, gEnableVRCasting);
#endif
...
EXTERN_VAR(bool, gUseVSyncCallback);
...
extern SxrServiceClientManager *gRVRManager;
#endif
...
extern bool gEnableCameraLayer;
#endif
...
int AddOneQuad(unsigned int currentIndex, unsigned int *indexData, unsigned int numIndices, unsigned int numRows, unsigned int numCols);
void GetWarpMatrix(glm::fquat origQuat, glm::mat4 & eyeProjectionMatrix, float adjustedTimeMs, glm::mat4 * pWarpMatrix);
#ifdef ENABLE_CAMERA
void InitializeCameraWarpMesh(SvrAsyncWarpResources* pWarpData);
#endif
...
// There are SXR_MAX_RENDER_LAYERS available each frame.
// These things are usually triple buffered.
// Double it cause we want to be safe.
#define MAX_VULKAN_MAP  (2 * 3 * SXR_MAX_RENDER_LAYERS)
...
extern const char svrOccluderVs[];
extern const char svrOccluderArrayVs[];
extern const char svrOccluderFs[];
...
//-----------------------------------------------------------------------------
void InitializeStencilMesh(SvrGeometry& stencilGeom, sxrWhichEye whichEye)
//-----------------------------------------------------------------------------
{
    LOGI("Initializing stencil mesh (%d vertices; %d indices)...", gNumHiddenMeshVerts, gNumHiddenMeshIndices);
...
//-----------------------------------------------------------------------------
void RenderStencilMesh(SvrAsyncWarpResources *pWarpData, sxrWhichEye whichEye, sxrTextureType imageType)
//-----------------------------------------------------------------------------
{
...
//-----------------------------------------------------------------------------
{
        ...
        if (gMeshOrderEnum == kMeshOrderLeftToRight)
        {
            for (unsigned int whichX = 0; whichX < numCols; whichX++)
            {
                for (unsigned int whichY = 0; whichY < numRows; whichY++)
                {
                    ...
                    numIndices += AddOneQuad(currentIndex, indexData, numIndices, numRows, numCols);
    ...
    LOGI("Creating Boundary Render Mesh: %d verts in a (%d rows x %d columns) grid", numVerts, numRows, numCols);
    ...
    if (pVertData == NULL)
    {
        LOGE("Unable to allocate %d bytes of memory for Boundary render mesh!", (int)(numElementsPerVert * numVerts * sizeof(float)));
        return;
    }
    ...
    for (uint32_t whichMesh = 0; whichMesh < kNumBoundaryMeshes; whichMesh++)
    {
        ...
        for (unsigned int whichX = 0; whichX < (numCols + 1); whichX++)
        {
            for (unsigned int whichY = 0; whichY < (numRows + 1); whichY++)
            {
                ...
                if (gLogMeshCreation)
                {
                    LOGI("    Vert [%d,%d]: Pos = (%0.5f, %0.5f, %0.5f); UV = (%0.5f, %0.5f);", whichX, whichY,
        ...
        pWarpData->BoundaryMeshGeom[whichMesh].Initialize(&BoundaryAttribs[0], numAttribs,
                                                        ...
                                                        pVertData, numElementsPerVert * numVerts * sizeof(float),
                                                        numVerts);
...
//-----------------------------------------------------------------------------
void RenderBoundaryMesh(SvrAsyncWarpResources *pWarpData, sxrWhichEye whichEye, sxrTextureType imageType)
//-----------------------------------------------------------------------------
{
    ...
    sxrHeadPoseState poseState = sxrGetPredictedHeadPose(0.0f);
    ...
    glm::mat4 leftProjMtx = glm::frustum(gLeftFrustum_Left, gLeftFrustum_Right, gLeftFrustum_Bottom, gLeftFrustum_Top, gLeftFrustum_Near, gLeftFrustum_Far);
    glm::mat4 rightProjMtx = glm::frustum(gRightFrustum_Left, gRightFrustum_Right, gRightFrustum_Bottom, gRightFrustum_Top, gRightFrustum_Near, gRightFrustum_Far);

    glm::mat4 eyeViewMat[2];

	// modified to use any config IPD & eye rotations for this device
    SvrGetEyeViewMatrices(poseState,
		false, // use head model?
        glm::vec3(0.0f, 0.0f, gLeftFrustum_Near), glm::vec3(0.0f, 0.0f, gRightFrustum_Near), // Mono
        glm::fquat(1.0f,0.0f, 0.0f, 0.0f), glm::fquat(1.0f, 0.0f, 0.0f, 0.0f), // no rotations (glm quat init = (w)(xyz)
		0.0f, 0.0f, // head hight & depth
		eyeViewMat[0], eyeViewMat[1]);  // matricies to set
    // glm::vec3 worldPos = glm::vec3(-eyeViewMat[0][3][0], -eyeViewMat[0][3][1], -eyeViewMat[0][3][2]);
    glm::vec3 worldPos = glm::vec3(-poseState.pose.position.x, -poseState.pose.position.y, -poseState.pose.position.z);
    ...
    glm::vec4 BoundaryColor = glm::vec4(gBoundaryMeshColorR, gBoundaryMeshColorG, gBoundaryMeshColorB, gBoundaryMeshColorA);
    ...
    for (uint32_t whichMesh = 0; whichMesh < kNumBoundaryMeshes; whichMesh++)
    {
        ...
        case kBoundaryMeshPX:
            ...
            {
                renderThisSide = true;
                lerpValue = gBoundaryVisabilityScale * fabs(worldPos.x - (gBoundaryZoneMaxX - gBoundaryVisabilityRadius)) / gBoundaryVisabilityRadius;
            }
            break;
        ...
        case kBoundaryMeshNX:
            ...
            {
                renderThisSide = true;
                lerpValue = gBoundaryVisabilityScale * fabs(worldPos.x - (gBoundaryZoneMinX + gBoundaryVisabilityRadius)) / gBoundaryVisabilityRadius;
            }
            break;
        ...
        case kBoundaryMeshPY:
            ...
            {
                renderThisSide = true;
                lerpValue = gBoundaryVisabilityScale * fabs(worldPos.y - (gBoundaryZoneMaxY - gBoundaryVisabilityRadius)) / gBoundaryVisabilityRadius;
            }
            break;
        ...
        case kBoundaryMeshNY:
            ...
            {
                renderThisSide = true;
                lerpValue = gBoundaryVisabilityScale * fabs(worldPos.y - (gBoundaryZoneMinY + gBoundaryVisabilityRadius)) / gBoundaryVisabilityRadius;
            }
            break;
        ...
        case kBoundaryMeshPZ:
            ...
            {
                renderThisSide = true;
                lerpValue = gBoundaryVisabilityScale * fabs(worldPos.z - (gBoundaryZoneMaxZ - gBoundaryVisabilityRadius)) / gBoundaryVisabilityRadius;
            }
            break;
        ...
        case kBoundaryMeshNZ:
            ...
            {
                renderThisSide = true;
                lerpValue = gBoundaryVisabilityScale * fabs(worldPos.z - (gBoundaryZoneMinZ + gBoundaryVisabilityRadius)) / gBoundaryVisabilityRadius;
            }
            break;
        }
        ...
        BoundaryColor = lerpValue * glm::vec4(gBoundaryMeshColorR, gBoundaryMeshColorG, gBoundaryMeshColorB, gBoundaryMeshColorA);
    ...
    if (!gForceDisplayBoundarySystem && collisionOccurred && whichEye == kLeftEye)
    {
        ...
        sxrEventData data;
        memset(&data, 0, sizeof(sxrEventData));
...
//-----------------------------------------------------------------------------
SxrResult L_InitializeBoundaryMeshSystem()
//-----------------------------------------------------------------------------
{
    SvrAsyncWarpResources* pWarpData = &gWarpData;
    ...
    if (!pWarpData->BoundaryMeshShader.Initialize(numVertStrings, vertShaderStrings, numFragStrings, fragShaderStrings, "svrBoundaryMeshVs", "svrBoundaryMeshFs"))
    {
    ...
    fragShaderStrings[numFragStrings++] = svrBoundaryMeshFs;
    if (!pWarpData->BoundaryMeshArrayShader.Initialize(numVertStrings, vertShaderStrings, numFragStrings, fragShaderStrings, "svrBoundaryMeshArrayVs", "svrBoundaryMeshFs"))
    {
...
//-----------------------------------------------------------------------------
SxrResult L_InitializeOccludeMeshSystem()
//-----------------------------------------------------------------------------
{
    SvrAsyncWarpResources* pWarpData = &gWarpData;

    //InitializeOccludeMesh();
    // This needs to be created on this thread so the render context is correct!
    ...
    //pWarpData->occludeMeshGeom.Initialize(&attribs[0], numAttribs,
    //    gCubeMeshIndices, gNumCubeMeshIndices,
    //    gCubeMeshVerts, numElementsPerVert * gNumCubeMeshVerts * sizeof(float), gNumCubeMeshVerts);
    ...
    LOGI("Initializing Occlude shader...");
    vertShaderStrings[numVertStrings++] = svrOccluderVs;
    fragShaderStrings[numFragStrings++] = svrOccluderFs;
    if (!pWarpData->occludeMeshShader.Initialize(numVertStrings, vertShaderStrings, numFragStrings, fragShaderStrings, "svrOccluderVs", "svrOccluderFs"))
    {
        LOGE("Failed to initialize OccludeMeshShader");
        return SXR_ERROR_UNKNOWN;
    }

    LOGI("Initializing Occlude (Array) shader...");
    numVertStrings = 0;
    numFragStrings = 0;
    vertShaderStrings[numVertStrings++] = svrOccluderArrayVs;
    fragShaderStrings[numFragStrings++] = svrOccluderFs;
    if (!pWarpData->occludeMeshArrayShader.Initialize(numVertStrings, vertShaderStrings, numFragStrings, fragShaderStrings, "svrOccluderArrayVs", "svrOccluderFs"))
    {
        LOGE("Failed to initialize OccludeMeshArrayShader");
        return SXR_ERROR_UNKNOWN;
    }
...
//-----------------------------------------------------------------------------
void RenderOccludeMesh(SvrAsyncWarpResources *pWarpData, sxrWhichEye whichEye, int frontFaceMode, sxrTextureType imageType, float* eyeProjMat, float* eyeViewMat, float* modelMat)
//-----------------------------------------------------------------------------
{
    if (pWarpData->occludeMeshGeom.GetIndexCount() == 0)
    {
        return;
    }

    Svr::SvrShader *pCurrentShader = &pWarpData->occludeMeshShader;
    if (imageType == kTypeTextureArray)
    {
        pCurrentShader = &pWarpData->occludeMeshArrayShader;
    ...
    {
        // Shader has not been set up
        LOGE("Unable to render occlude mesh: Occlude Shader NOT initialized!");
        return;
    }
    ...
    {
        // Shader is not a valid program
        LOGE("Unable to render occlude mesh: Occlude Shader NOT valid!");
        return;
    }
    ...
    // Vertex order
    GLenum vertexWinding = frontFaceMode == 0 ? GL_CCW : GL_CW;
    GL(glFrontFace(vertexWinding)); 
    ...
    // ... set uniforms ...
    pCurrentShader->SetUniformMat4fv(1, 1, eyeProjMat);

    pCurrentShader->SetUniformMat4fv(2, 1, eyeViewMat);

    pCurrentShader->SetUniformMat4fv(3, 1, modelMat);

    glm::vec4 meshColor = glm::vec4(gOcclusionMeshColorR, gOcclusionMeshColorG, gOcclusionMeshColorB, gOcclusionMeshColorA);
    pCurrentShader->SetUniformVec4(4, meshColor);
    // pCurrentShader->SetUniformSampler("srcTex", imageHandle, GL_TEXTURE_2D, 0);

    // ... no textures ...

    // ... render the mesh
    pWarpData->occludeMeshGeom.Submit();
...
//-----------------------------------------------------------------------------
{
    return gOcclusionMeshEnabled == true;
}
...
//-----------------------------------------------------------------------------
{
    if (gOcclusionMeshEnabled == false)
    {
    ...
    if (pWarpData->occludeMeshGeom.GetIndexCount() > 0 || pWarpData->occludeMeshGeom.GetVertexCount() > 0)
    {
        pWarpData->occludeMeshGeom.Destroy();
    }
    ...
    if (numMeshVertices > 0 && numMeshIndices > 0 && meshVertices != NULL && meshIndices != NULL)
    {
        pWarpData->occludeMeshGeom.Initialize(&attribs[0], numAttribs,
            meshIndices, numMeshIndices,
            meshVertices, numElementsPerVert * numMeshVertices * sizeof(float), numMeshVertices);
    }
...
//-----------------------------------------------------------------------------
SxrResult sxrOccludeEye(sxrWhichEye whichEye, float *pEyeProj, float *pEyeView, float *pModelMat, int frontFaceMode, sxrTextureType imageType)
//-----------------------------------------------------------------------------
{
    if (gOcclusionMeshEnabled == false)
    {
    ...
    {
        LOGE("sxrOccludeEye Failed: Called when not in VR mode!");
        return SXR_ERROR_VRMODE_NOT_STARTED;
    }

    if (pModelMat == NULL)
    {
        pModelMat = glm::value_ptr(gAppContext->modeContext->qvrTransformMat);
    }
    else
    {
        glm::mat4 transformMat = glm::make_mat4(pModelMat) * gAppContext->modeContext->qvrTransformMat;
        pModelMat = glm::value_ptr(transformMat);
    }

    SvrAsyncWarpResources* pWarpData = &gWarpData;

    if (pWarpData->occludeMeshShader.GetShaderId() == 0)
        L_InitializeOccludeMeshSystem();

    RenderOccludeMesh(&gWarpData, whichEye, frontFaceMode, imageType, pEyeProj, pEyeView, pModelMat);

    return SXR_ERROR_NONE;
}

//-----------------------------------------------------------------------------
SxrResult sxrBeginEye(sxrWhichEye whichEye, sxrTextureType imageType)
//-----------------------------------------------------------------------------
{
    if(gAppContext == NULL )
    {
    ...
    if (gStencilMeshEnabled && pWarpData->stencilShader.GetShaderId() == 0)
    {
        ...
        if (!pWarpData->stencilShader.Initialize(numVertStrings, vertShaderStrings, numFragStrings, fragShaderStrings, "svrStencilVs", "svrStencilFs"))
        {
        ...
        fragShaderStrings[numFragStrings++] = svrStencilFs;
        if (!pWarpData->stencilArrayShader.Initialize(numVertStrings, vertShaderStrings, numFragStrings, fragShaderStrings, "svrStencilArrayVs", "svrStencilFs"))
        {
...
//-----------------------------------------------------------------------------
SxrResult sxrEndEye(sxrWhichEye whichEye, sxrTextureType imageType)
//-----------------------------------------------------------------------------
{
    if (gAppContext == NULL)
    {
...
//-----------------------------------------------------------------------------
SXRP_EXPORT void sxrEnableBoundarySystem(bool enableBoundary)
//-----------------------------------------------------------------------------
{
    // For now, the Boundary system means the Boundary mesh
...
//-----------------------------------------------------------------------------
SXRP_EXPORT void sxrForceDisplayBoundarySystem(bool forceDisplayBoundary)
//-----------------------------------------------------------------------------
{
...
//-----------------------------------------------------------------------------
{
    ...
    switch(eglError)
    {
    case EGL_SUCCESS:   // The last function succeeded without error.
        LOGI("    => EGL_SUCCESS");
        break;

    case EGL_NOT_INITIALIZED:   // EGL is not initialized, or could not be initialized, for the specified EGL display connection.
        LOGI("    => EGL_NOT_INITIALIZED");
        break;

    case EGL_BAD_ACCESS:    // EGL cannot access a requested resource(for example a context is bound in another thread).
        LOGI("    => EGL_BAD_ACCESS");
        break;
    ...
    case EGL_BAD_CURRENT_SURFACE:   // The current surface of the calling thread is a window, pixel buffer or pixmap that is no longer valid.
        LOGI("    => EGL_BAD_CURRENT_SURFACE");
        break;

    case EGL_BAD_DISPLAY:   // An EGLDisplay argument does not name a valid EGL display connection.
        LOGI("    => EGL_BAD_DISPLAY");
        break;

    case EGL_BAD_SURFACE:   // An EGLSurface argument does not name a valid surface(window, pixel buffer or pixmap) configured for GL rendering.
        LOGI("    => EGL_BAD_SURFACE");
        break;

    case EGL_BAD_MATCH: // Arguments are inconsistent(for example, a valid context requires buffers not supplied by a valid surface).
        LOGI("    => EGL_BAD_MATCH");
        break;

    case EGL_BAD_PARAMETER: // One or more argument values are invalid.
        LOGI("    => EGL_BAD_PARAMETER");
        break;

    case EGL_BAD_NATIVE_PIXMAP: // A NativePixmapType argument does not refer to a valid native pixmap.
        LOGI("    => EGL_BAD_NATIVE_PIXMAP");
        break;
    ...
    case EGL_CONTEXT_LOST:  // A power management event has occurred.The application must destroy all contexts and reinitialise OpenGL ES state and objects to continue rendering.
        LOGI("    => EGL_CONTEXT_LOST");
        break;

    default:
        LOGI("    => Unknown (0x%x)", eglError);
        break;
    }
...
//-----------------------------------------------------------------------------
{
    ...
    if (largestPBuffer == -1)
    {
        ...
        LOGI("Config ID of eye surface: %d", gAppContext->modeContext->eyeRenderOrigConfigId);

        EGLConfig pbufferEyeSurfaceConfig = (EGLConfig)0;

#ifdef NOT_READING_CONFIG_FROM_CONTEXT
        EGLint configId;
        if (eglQueryContext(eyeDisplay, gAppContext->modeContext->eyeRenderContext, EGL_CONFIG_ID, &configId))
        {
            LOGI("Config ID of eye context: %d", configId);
            if (configId == 0)
            {
                LOGE("    0 is not a valid config ID!");
            }
        }
        else
        {
            LOGE("Unable to read config ID for eye render context!");
            L_LogEglError();
        }
#endif // NOT_READING_CONFIG_FROM_CONTEXT

        EGLint numConfigs;
        EGLConfig configs[1024];
        eglGetConfigs(eyeDisplay, configs, 1024, &numConfigs);
        LOGI("Found %d possible EGL configs", numConfigs);
        for (int i = 0; i < numConfigs; i++)
        {
            EGLint tmpConfigId;
            eglGetConfigAttrib(eyeDisplay, configs[i], EGL_CONFIG_ID, &tmpConfigId);
            // LOGI("    Config #%d: %d", i, tmpConfigId);
            if (tmpConfigId == gAppContext->modeContext->eyeRenderOrigConfigId)
            {
                LOGI("Warp Context Replace Config #: %d", i);
                pbufferEyeSurfaceConfig = configs[i];
            }
        }

        if (pbufferEyeSurfaceConfig == (EGLConfig)0)
        {
            LOGE("Unable to find current eye surface config!");
            return false;
        }
        ...
        if (gAppContext->modeContext->isProtectedContent)
        {
            ...
            LOG_EGL_RESOURCE("%s:%i:threadid=%i, eglCreatePbufferSurface(eyeDisplay=%p, pbufferEyeSurfaceConfig=%p)=eyeRenderWarpSurface=%p",
                __FILE__, __LINE__, gettid(), eyeDisplay, pbufferEyeSurfaceConfig, gAppContext->modeContext->eyeRenderWarpSurface);
        }
        else
        {
            const EGLint surfaceAttribs[] =
        ...
        LOG_EGL_RESOURCE("%s:%i:threadid=%i, eglMakeCurrent(eyeDisplay=%p, gAppContext->modeContext->eyeRenderWarpSurface=%p, gAppContext->modeContext->eyeRenderWarpSurface=%p, gAppContext->modeContext->eyeRenderContext=%p)=1", 
            __FILE__, __LINE__, gettid(), eyeDisplay, gAppContext->modeContext->eyeRenderWarpSurface, gAppContext->modeContext->eyeRenderWarpSurface, gAppContext->modeContext->eyeRenderContext);
        ...
        {
            LOGE("svrUpdateEyeContextSurface: eglMakeCurrent failed for eye context, 0x%x", eglGetError());
...
//-----------------------------------------------------------------------------
{
    ...
    if (eglGetCurrentContext() == 0 && eglGetCurrentSurface(EGL_DRAW) == 0)
    {
        ...
        EGLint major, minor;
        const EGLBoolean eglInitializeResult = eglInitialize(gVkConsumer_eglDisplay, &major, &minor);
        LOGI("EGL version %d.%d", major, minor);
        LOG_EGL_RESOURCE("%s:%i:threadid=%i, eglInitialize(display=%p)=%i", __FILE__, __LINE__, gettid(), gVkConsumer_eglDisplay, eglInitializeResult);
    }
    ...
    if (true)//di.deviceOSVersion < 24)
    {
        ...
        if (gAppContext->modeContext->eyeRenderOrigSurface != EGL_NO_SURFACE)
        {
            ...
            const EGLBoolean destroySurfaceResult = eglDestroySurface(display, gAppContext->modeContext->eyeRenderOrigSurface);
            LOG_EGL_RESOURCE("%s:%i:threadid=%i, eglDestroySurface(display=%p, eyeRenderOrigSurface=%p)=%i", __FILE__, __LINE__, gettid(), display, gAppContext->modeContext->eyeRenderOrigSurface, destroySurfaceResult);
        ...
        LOG_EGL_RESOURCE("%s:%i:threadid=%i, eglCreateWindowSurface(display=%p, config=%p, gAppContext->modeContext->nativeWindow=%p)=warpRenderSurface=%p", __FILE__, __LINE__, gettid(), display, config, gAppContext->modeContext->nativeWindow, surface);
    ...
    const int32_t setBuffersGeometryResult = ANativeWindow_setBuffersGeometry(gAppContext->modeContext->nativeWindow, di.displayWidthPixels, di.displayHeightPixels, format);
    LOG_EGL_RESOURCE("%s:%i:threadid=%i, ANativeWindow_setBuffersGeometry(gAppContext->modeContext->nativeWindow=%p (0 means success)=%i", __FILE__, __LINE__, gettid(), gAppContext->modeContext->nativeWindow, setBuffersGeometryResult);
...
//-----------------------------------------------------------------------------
{
    ...
    EGLBoolean makeCurrentResult = eglMakeCurrent(display, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT);
    LOG_EGL_RESOURCE("%s:%i:threadid=%i, eglMakeCurrent(display=%p, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT)=%i", __FILE__, __LINE__, gettid(), display, makeCurrentResult);
    ...
    {
        const EGLBoolean destroySurfaceResult = eglDestroySurface(gVkConsumer_eglDisplay, gVkConsumer_eglSurface);
        LOG_EGL_RESOURCE("%s:%i:threadid=%i, eglDestroySurface(display=%p, gVkConsumer_eglSurface=%p)=%i", __FILE__, __LINE__, gettid(), gVkConsumer_eglDisplay, gVkConsumer_eglSurface, destroySurfaceResult);
    }
    gVkConsumer_eglSurface = EGL_NO_SURFACE;

    if (gVkConsumer_eglDisplay != EGL_NO_DISPLAY && gVkConsumer_eglContext != EGL_NO_CONTEXT)
    {
        const EGLBoolean destroyContextResult = eglDestroyContext(gVkConsumer_eglDisplay, gVkConsumer_eglContext);
        LOG_EGL_RESOURCE("%s:%i:threadid=%i, eglDestroyContext(display=%p, context=%p)=%i", __FILE__, __LINE__, gettid(), gVkConsumer_eglDisplay, gVkConsumer_eglContext, destroyContextResult);
    }
    ...
    {
        const EGLBoolean destroySurfaceResult = eglDestroySurface(display, gAppContext->modeContext->warpRenderSurface);
        LOG_EGL_RESOURCE("%s:%i:threadid=%i, eglDestroySurface(display=%p, warpRenderSurface=%p)=%i", __FILE__, __LINE__, gettid(), display, gAppContext->modeContext->warpRenderSurface, destroySurfaceResult);
        
        gAppContext->modeContext->warpRenderSurface = EGL_NO_SURFACE;
        
        const EGLBoolean destroyContextResult = eglDestroyContext(display, gAppContext->modeContext->warpRenderContext);
        LOG_EGL_RESOURCE("%s:%i:threadid=%i, eglDestroyContext(display=%p, warpRenderContext=%p)=%i", __FILE__, __LINE__, gettid(), display, gAppContext->modeContext->warpRenderContext, destroyContextResult);

        gAppContext->modeContext->warpRenderContext = EGL_NO_CONTEXT;

        const EGLBoolean eglTerminateResult = eglTerminate(display);
        LOG_EGL_RESOURCE("%s:%i:threadid=%i, eglTerminate(display=%p)=%i", __FILE__, __LINE__, gettid(), display, eglTerminateResult);
    }
    else
    {
...
//-----------------------------------------------------------------------------
void GenerateWarpCoords(float *pBaseUV, 
                        float *pWarpR, 
                        float *pWarpG, 
                        float *pWarpB, 
                        svrMeshDistortion distortType, 
                        glm::vec4 lensScaleOffset,
                        float *pSamplePoint, // pSamplePoint is 0..1  along image width, height
                        bool doDisplayDistortion)
//-----------------------------------------------------------------------------
{
    ...
    DistCoordX -= lensScaleOffset[2]; // gLensOffsetX;
    DistCoordY -= lensScaleOffset[3]; // gLensOffsetY;

    DistCoordX *= lensScaleOffset[0]; // gLensScale;
    DistCoordY *= lensScaleOffset[1]; // gLensScale;
    ...
    /*
     * Calculate Radius of the pixel.
     */
    float radius;
    if (pSamplePoint != NULL) // pSamplePoint is 0..1  along image width, height
    {
        /*
         * This uses the focal length of the display lens, converting to 3D
         * camera space.  The range is not (necessarily) -1 .. +1. This solves
         * the aspect ratio vs. scale problem.
         */
        float screenWidth = gAppContext->deviceInfo.displayWidthPixels;
        float screenHeight = gAppContext->deviceInfo.displayHeightPixels;
        float screenFakeFocalMM = gScreenWidthMM / 4.f; // not the real focal length, just for backwards compatibility of lens K coeffs.
        float screenFocal = screenFakeFocalMM * (screenWidth /  gScreenWidthMM); // focal length in pixels

        float displayWidth = screenWidth / 2.f;    // for one eye. We'll call one eye's half-screen a "Display"
        float displayHeight = screenHeight;

        float displayX = pSamplePoint[0] * displayWidth;
        float displayY = pSamplePoint[1] * displayHeight;
	    
        float displayCenterX = displayWidth / 2.f;
        float displayCenterY = displayHeight / 2.f;
	    
        displayX = (displayX - displayCenterX) / screenFocal;
        displayY = (displayY - displayCenterY) / screenFocal;
    
        radius = sqrtf(displayX*displayX + displayY*displayY);
        Radius1 = radius; // use the corrected camera space radius
    }
    ...
    if (doDisplayDistortion == false)
    {
        /*
         * We use this for kBarrelDistortion, because we use XYZ, not UV to warp the image.
         * We still enter this function to get UVs for chromatic aberration correction.
         */
        DistortionScale = 1.f; 
    }
    ...
    for (int WhichCoord = 0; WhichCoord < 3; WhichCoord++)
    {
        ...
        ActualCoordX[WhichCoord] /= lensScaleOffset[0]; // gLensScale;
        ActualCoordY[WhichCoord] /= lensScaleOffset[1]; // gLensScale;

        // Adjust origin to center of lens rather than center of screen
        ActualCoordX[WhichCoord] += lensScaleOffset[2]; // gLensOffsetX;
        ActualCoordY[WhichCoord] += lensScaleOffset[3]; // gLensOffsetY;
...
//-----------------------------------------------------------------------------
{
    ...
    if (gLogMeshCreation)
        LOGI("World Pos: Theta/Phi = %0.2f / %0.2f; (X, Y, Z) = (%0.2f, %0.2f, %0.2f)", thisTheta, thisPhi, xPos, yPos, zPos);
    ...
    else
    {
        ...
        GenerateWarpCoords(baseUV, tempVert.TexCoordR, tempVert.TexCoordG, tempVert.TexCoordB, distortType, lensScaleOffset, NULL, true);
        ...
        if (gLogMeshCreation)
            LOGI("    R: (X, Y, Z) = (%0.2f, %0.2f, %0.2f)", vertData[whichVert].TexCoordR[0], vertData[whichVert].TexCoordR[1], vertData[whichVert].TexCoordR[2]);
        ...
        if (gLogMeshCreation)
            LOGI("    G: (X, Y, Z) = (%0.2f, %0.2f, %0.2f)", vertData[whichVert].TexCoordG[0], vertData[whichVert].TexCoordG[1], vertData[whichVert].TexCoordG[2]);
        ...
        if (gLogMeshCreation)
            LOGI("    B: (X, Y, Z) = (%0.2f, %0.2f, %0.2f)", vertData[whichVert].TexCoordB[0], vertData[whichVert].TexCoordB[1], vertData[whichVert].TexCoordB[2]);
    }
...
//-----------------------------------------------------------------------------
glm::mat4 CalculateProjectionMatrix2()
//-----------------------------------------------------------------------------
{
...
//-----------------------------------------------------------------------------
void InitializeWarpMesh(SvrGeometry& warpGeom, svrWarpMeshArea whichMesh, unsigned int numRows, unsigned int numCols, svrMeshLayout &meshLayout, glm::vec4 meshScaleOffset, svrMeshMode meshMode, svrMeshDistortion distortType, glm::vec4 lensScaleOffset)
//-----------------------------------------------------------------------------
{
    // Make sure we are at least 2x2
    if (numRows < 2)
    ...
    if (gUseDistortionMeshData == true && whichMesh!= kMeshLeft_Camera && whichMesh!=kMeshRight_Camera)
    {
        sxrDeviceInfo deviceInfo = sxrGetDeviceInfo();

        if (whichMesh == kMeshLeft ||
            whichMesh == kMeshUL ||
            whichMesh == kMeshLL ) 
        {
            LOGI("Creating Projection Layer Mesh: Left %d rows x %d columns grid", numRows, numCols);
            bool result = gSxrDistortionMesh.CreateProjectionLayerMesh(warpGeom, kLeftEye, whichMesh, numRows, numCols);
            if (result == true)
            {
                return; // return if custom mesh worked. Otherwise keep going, make the usual mesh.
            }
        } else
        if (whichMesh == kMeshRight ||
            whichMesh == kMeshUR ||
            whichMesh == kMeshLR )
        {
            LOGI("Creating Projection Layer Mesh: Right %d rows x %d columns grid", numRows, numCols);
            bool result = gSxrDistortionMesh.CreateProjectionLayerMesh(warpGeom, kRightEye, whichMesh, numRows, numCols);
            if (result == true)
            {
                return; // return if custom mesh worked. Otherwise keep going, make the usual mesh.
            }
        } else
        if (whichMesh == kMeshLeft_Sphere ||
            whichMesh == kMeshUL_Sphere ||
            whichMesh == kMeshLL_Sphere)
        {
            LOGI("Creating Equirect Layer Mesh: Left %d rows x %d columns grid", numRows, numCols);
            bool result = gSxrDistortionMesh.CreateEquirectLayerMesh(warpGeom, kLeftEye, whichMesh, numRows, numCols, deviceInfo);
            if (result == true)
            {
                return; // return if custom mesh worked. Otherwise keep going, make the usual mesh.
            }
        } else
        if (whichMesh == kMeshRight_Sphere ||
            whichMesh == kMeshUR_Sphere ||
            whichMesh == kMeshLR_Sphere)
        {
            LOGI("Creating Equirect Layer Mesh: Right %d rows x %d columns grid", numRows, numCols);
            bool result = gSxrDistortionMesh.CreateEquirectLayerMesh(warpGeom, kRightEye, whichMesh, numRows, numCols, deviceInfo);
            if (result == true)
            {
                return; // return if custom mesh worked. Otherwise keep going, make the usual mesh.
            }
        } else
        if (whichMesh == kMeshOverlay)
        {
            LOGI("Creating Overlay Layer Mesh: Cache %d rows x %d columns grid", numRows, numCols);
            gSxrDistortionMesh.InitQuadLayerMeshes(numRows, numCols);
            return; // return if custom mesh worked. Otherwise keep going, make the usual mesh.
        }
    }

    unsigned int numVerts = ((numRows + 1) * (numCols + 1)); // xyz = 1 vert
    ...
    if (gLogMeshCreation && distortType == kSphereDistortion)
    {
    ...
    if (distortType == kSphereDistortion)
    {
        ...
        case kMeshLeft_Sphere:
            ...
            minTheta = -leftThetaA;
            deltaTheta = (leftThetaA + leftThetaB) / (float)numCols;    // These are both positive angles
            minPhi = -leftPhiB;
            deltaPhi = (leftPhiA + leftPhiB) / (float)numRows;          // These are both positive angles
        ...
        case kMeshRight_Sphere:
            ...
            deltaTheta = (rightThetaA + rightThetaB) / (float)numCols;  // These are both positive angles
            minPhi = -rightPhiB;
            deltaPhi = (rightPhiA + rightPhiB) / (float)numRows;        // These are both positive angles
            sphereScaleOffset = glm::vec4(0.5f, 1.0f, 0.5f, 0.0f);
        ...
        default:
            LOGE("ERROR! Uknown sphere mesh specified (%d)!  Defaulting to normal barrel distortion.", (int)whichMesh);
            break;
        }

        if (gLogMeshCreation && isSphericalMesh)
        {
            LOGI("Creating spherical mesh. Theta = [%0.2f, %0.2f]; Phi = [%0.2f, %0.2f]",   ToDegrees * minTheta, 
    ...
    //Ensure camera mesh has been inited
    //Do Enable Camera check
    //Single Pass Check ? this is only called in Single pass
    //Ensure camera mesh size = this mesh size
    ...
    for (unsigned int whichX = 0; whichX < (numCols + 1); whichX++)
    {
        for (unsigned int whichY = 0; whichY < (numRows + 1); whichY++)
        {
            ...
            /*
             * The tempPos values are -1...1, and do take mesh splitting into account.
             * samplePoint is 0..1.
             */
            samplePoint[0] = (tempPos.x / 2.f) + 0.5f;
            samplePoint[1] = (tempPos.y / 2.f) + 0.5f;

            // ************************
            // Position Values
            // ************************
            float DistortionScale = 1.0f;
            if (distortType == kBarrelDistortion && gLensPolyUseXYZ == true)
            {
                float screenWidth = gAppContext->deviceInfo.displayWidthPixels;
                float screenHeight = gAppContext->deviceInfo.displayHeightPixels;
                float screenFakeFocalMM = gScreenWidthMM / 4.f;                         // not the real focal length, just for backwards compatibility of lens K coeffs.
                ...
                float displayX = samplePoint[0] * displayWidth;
                float displayY = samplePoint[1] * displayHeight;
                ...
                float Radius1 = sqrtf(displayX * displayX + displayY * displayY);
                float Radius2 = Radius1 * Radius1;
                float Radius3 = Radius2 * Radius1;
                float Radius4 = Radius3 * Radius1;
                float Radius5 = Radius4 * Radius1;
                ...
                DistortionScale += gLensXYZPolyK0;
                DistortionScale += gLensXYZPolyK1 * Radius1;
                DistortionScale += gLensXYZPolyK2 * Radius2;
                DistortionScale += gLensXYZPolyK3 * Radius3;
                DistortionScale += gLensXYZPolyK4 * Radius4;
                DistortionScale += gLensXYZPolyK5 * Radius5;
                DistortionScale += gLensXYZPolyK6 * Radius6;
            }

                vertData[whichVert].Position[0] = tempPos.x * DistortionScale;
                vertData[whichVert].Position[1] = tempPos.y * DistortionScale;
                ...
                if (isSphericalMesh)
                {
                    ...
                    L_SetOneSphereVert(vertData, whichVert, thisTheta, thisPhi, baseUV, sphereScaleOffset, thisProjMtx, thisInvProjMtx, distortType, lensScaleOffset);
                ...
                default:
                    ...
                    GenerateWarpCoords(baseUV, vertData[whichVert].TexCoordR, vertData[whichVert].TexCoordG, vertData[whichVert].TexCoordB, distortType, lensScaleOffset, NULL, true);
                ...
                case kBarrelDistortion:
                    /*
                 * Warp the base UV texture coordinates based on display lens properties.
                 * The last boolean is whether we use UVs to warp mesh for undistort.
                 * Regardless of gLensPolyUseXYZ, UV's are *still* used for chromatic
                 * aberration correction, so still call this function.
                 */
                    GenerateWarpCoords(baseUV,
                                       vertData[whichVert].TexCoordR,
                                       vertData[whichVert].TexCoordG,
                                       vertData[whichVert].TexCoordB,
                                       distortType,
                                       lensScaleOffset,
                                       samplePoint,
                                       !gLensPolyUseXYZ); // if warping XYZ coords, then don't warp coords
 #ifdef ENABLE_CAMERA
                    if (whichMesh == kMeshLeft_Camera || whichMesh == kMeshRight_Camera)
                    {
                        float texCoordR[4];
                        float texCoordG[4];
                        float texCoordB[4];
                        float mesh_width =  gAppContext->deviceInfo.displayWidthPixels/2.f;
                        float mesh_height = gAppContext->deviceInfo.displayHeightPixels/1.f;
                        int whicheye;
                        whichMesh == kMeshLeft_Camera ? whicheye = kLeftEye : whicheye = kRightEye;
                        gSvrCameraManager.GetCameraWarpCoords(vertData[whichVert].TexCoordR, texCoordR, mesh_width, mesh_height, whicheye);
                        gSvrCameraManager.GetCameraWarpCoords(vertData[whichVert].TexCoordG, texCoordG, mesh_width, mesh_height,whicheye);
                        gSvrCameraManager.GetCameraWarpCoords(vertData[whichVert].TexCoordB, texCoordB, mesh_width, mesh_height,whicheye);
                        
                        //fill in camera mesh coordinates
                        vertData[whichVert].TexCoordR[2] = texCoordR[0];
                        vertData[whichVert].TexCoordR[3] = texCoordR[1];
                        vertData[whichVert].TexCoordG[2] = texCoordG[0];
                        vertData[whichVert].TexCoordG[3] = texCoordG[1];
                        vertData[whichVert].TexCoordB[2] = texCoordB[0];
                        vertData[whichVert].TexCoordB[3] = texCoordB[1];
                    }
#endif
                    if (!gDisableLensCorrection)
    ...
    if (gLogMeshCreation)
    {
        for (unsigned int whichVert = 0; whichVert < numVerts; whichVert++)
        {
            ...
            if (isSphericalMesh)
            {
                LOGI("Vert %d of %d: Pos = (%0.5f, %0.5f, %0.5f); UV = (%0.5f, %0.5f);", whichVert + 1, numVerts, vertData[whichVert].Position[0], vertData[whichVert].Position[1], vertData[whichVert].Position[2], vertData[whichVert].TexCoordR[0], vertData[whichVert].TexCoordR[1]);
            }
            else
            {
                LOGI("Vert %d of %d: Pos = (%0.5f, %0.5f, %0.5f); UV = (%0.5f, %0.5f);", whichVert + 1, numVerts, vertData[whichVert].Position[0], vertData[whichVert].Position[1], vertData[whichVert].Position[2], vertData[whichVert].TexCoordR[0], vertData[whichVert].TexCoordR[1]);
...
//-----------------------------------------------------------------------------
void UpdateOverlayGeometry(SvrGeometry &overlayGeometry, svrMeshLayout &meshLayout, glm::vec4 meshScaleOffset, glm::vec4 lensScaleOffset)
//-----------------------------------------------------------------------------
{
    ...
    for (unsigned int whichX = 0; whichX < (numCols + 1); whichX++)
    {
        for (unsigned int whichY = 0; whichY < (numRows + 1); whichY++)
        {
            ...
            GenerateWarpCoords(baseUV, tempVert.TexCoordR, tempVert.TexCoordG, tempVert.TexCoordB, kBarrelDistortion, lensScaleOffset, NULL, true);

            gOverlayVertData[whichVert].TexCoordR[0] = tempUV.x + uRatio * (tempVert.TexCoordR[0] - tempPos.x);
            gOverlayVertData[whichVert].TexCoordR[1] = tempUV.y + vRatio * (tempVert.TexCoordR[1] - tempPos.y);

            gOverlayVertData[whichVert].TexCoordG[0] = tempUV.x + uRatio * (tempVert.TexCoordG[0] - tempPos.x);
            gOverlayVertData[whichVert].TexCoordG[1] = tempUV.y + vRatio * (tempVert.TexCoordG[1] - tempPos.y);

            gOverlayVertData[whichVert].TexCoordB[0] = tempUV.x + uRatio * (tempVert.TexCoordB[0] - tempPos.x);
            gOverlayVertData[whichVert].TexCoordB[1] = tempUV.y + vRatio * (tempVert.TexCoordB[1] - tempPos.y);
            ...
            // [0,1] => [-1,1]
            gOverlayVertData[whichVert].TexCoordR[0] = 2.0f * gOverlayVertData[whichVert].TexCoordR[0] - 1.0f;
            gOverlayVertData[whichVert].TexCoordR[1] = 2.0f * gOverlayVertData[whichVert].TexCoordR[1] - 1.0f;
            gOverlayVertData[whichVert].TexCoordG[0] = 2.0f * gOverlayVertData[whichVert].TexCoordG[0] - 1.0f;
            gOverlayVertData[whichVert].TexCoordG[1] = 2.0f * gOverlayVertData[whichVert].TexCoordG[1] - 1.0f;
            gOverlayVertData[whichVert].TexCoordB[0] = 2.0f * gOverlayVertData[whichVert].TexCoordB[0] - 1.0f;
            gOverlayVertData[whichVert].TexCoordB[1] = 2.0f * gOverlayVertData[whichVert].TexCoordB[1] - 1.0f;
            ...
            // GenerateWarpCoords(baseUV, gOverlayVertData[whichVert].TexCoordR, gOverlayVertData[whichVert].TexCoordG, gOverlayVertData[whichVert].TexCoordB, kBarrelDistortion, lensScaleOffset);
            ...
            // Handle Vignette
            if (gMeshVignette)
                L_AdjustForVignette(&gOverlayVertData[whichVert], scaleFactor * gLensPolyUnitRadius);

            // Now adjust where we want it to be on screen
            // LOGI("    ScaleOffset: (%0.5f, %0.5f, %0.5f, %0.5f)", meshScaleOffset.x, meshScaleOffset.y, meshScaleOffset.z, meshScaleOffset.w);
            gOverlayVertData[whichVert].Position[0] *= meshScaleOffset.x;
            gOverlayVertData[whichVert].Position[0] += meshScaleOffset.z * gOverlayVertData[whichVert].Position[3];
            gOverlayVertData[whichVert].Position[1] *= meshScaleOffset.y;
            gOverlayVertData[whichVert].Position[1] += meshScaleOffset.w * gOverlayVertData[whichVert].Position[3];
        }   // Each Column
    }   // Each Row
    // LOGE("****************************************");
    ...
    if (gLogMeshCreation)
    {
        ...
        for (unsigned int whichVert = 0; whichVert < numVerts; whichVert++)
        {
            LOGI("Vert %d of %d: Pos = (%0.5f, %0.5f, %0.5f, %0.5f); UV = (%0.5f, %0.5f);", whichVert + 1, 
                                                                                            numVerts, 
    ...
    overlayGeometry.Update(gOverlayVertData, sizeof(warpMeshVert) * numVerts, numVerts);
...
//-----------------------------------------------------------------------------
{
    ...
    if (pWarpData->pWarpShaderMap[whichShader] & kWarpSubsampled)
    {
        vertShaderStrings[numVertStrings++] = UBER_SUBSAMPLED_EXT_STRING;
        fragShaderStrings[numFragStrings++] = UBER_SUBSAMPLED_EXT_STRING;
    }

#ifdef ENABLE_CAMERA
    if (pWarpData->pWarpShaderMap[whichShader] & kWarpCameraImage)
    {
        QVRCAMERA_FRAME_FORMAT cameraFormat = gSvrCameraManager.GetCameraFrameFormat();
        if (cameraFormat != QVRCAMERA_FRAME_FORMAT_RAW16_MONO)
        {
            vertShaderStrings[numVertStrings++] = UBER_CAMERA_EXT_STRING;
            fragShaderStrings[numFragStrings++] = UBER_CAMERA_EXT_STRING;
        }
    }
#endif
    ...
    // If color planes are enabled, all shaders support it (not a shader flag)
    if (gSupportColorPlaneWarp)
    {
        LOGI("Adding Color Plane support to shader: 0x%08x", pWarpData->pWarpShaderMap[whichShader]);
        vertShaderStrings[numVertStrings++] = UBER_COLOR_PLANE_STRING;
        fragShaderStrings[numFragStrings++] = UBER_COLOR_PLANE_STRING;
    }
    ...
    if (pWarpData->pWarpShaderMap[whichShader] & kWarpCameraImage)
    {
        ...
        case QVRCAMERA_FRAME_FORMAT_RAW16_MONO:
            if (gSvrCameraManager.IsMonoFloat16() == true)
            {
                // 16 bit float monochrome
                vertShaderStrings[numVertStrings++] = UBER_CAMERA_MONO_F16_STRING;
                fragShaderStrings[numFragStrings++] = UBER_CAMERA_MONO_F16_STRING;
            }
            else
            {
                // 16 bit integer monochrome
                vertShaderStrings[numVertStrings++] = UBER_CAMERA_MONO_INT16_STRING;
                fragShaderStrings[numFragStrings++] = UBER_CAMERA_MONO_INT16_STRING;
            }
            break;
        ...
        case QVRCAMERA_FRAME_FORMAT_Y8:
            ...
            vertShaderStrings[numVertStrings++] = UBER_CAMERA_MONO_8_10BIT_STRING;
            fragShaderStrings[numFragStrings++] = UBER_CAMERA_MONO_8_10BIT_STRING;
            break;

        case QVRCAMERA_FRAME_FORMAT_YUV420:
            // RGB color pixel texture - GL_LINEAR texture filtering
            if(gSvrCameraManager.IsPFDSupported())
            {
                vertShaderStrings[numVertStrings++] = UBER_CAMERA_COLOR_SWAP_STRING;
                fragShaderStrings[numFragStrings++] = UBER_CAMERA_COLOR_SWAP_STRING;
            }
            else
            {
                vertShaderStrings[numVertStrings++] = UBER_CAMERA_COLOR_STRING;
                fragShaderStrings[numFragStrings++] = UBER_CAMERA_COLOR_STRING;
            }
            break;

        default:
            LOGE("Failed to initialize shader for camera layer.");
            return false;
        }
    }
#endif
    ...
    if (pWarpData->pWarpShaderMap[whichShader] & kWarpSubsampled)
    {
        vertShaderStrings[numVertStrings++] = UBER_SUBSAMPLED_STRING;
        fragShaderStrings[numFragStrings++] = UBER_SUBSAMPLED_STRING;
    }
...
//-----------------------------------------------------------------------------
{
    ...
    glm::vec4 lensScaleOffset = glm::vec4(1.0f, 1.0f, 0.0f, 0.0f);
    ...
    if (gUseDistortionMeshData)
    {
        LOGI("gSxrDistortionMesh.Initialize() ...");
        bool result = gSxrDistortionMesh.Initialize(
        ...
        if (result == false)
        {
            LOGE("gSxrDistortionMesh.Initialize() Failed!");
            gUseDistortionMeshData = false;
        }
    }
    
    
#ifdef ENABLE_CAMERA
    InitializeCameraWarpMesh(pWarpData);
#endif
    //switch (gMeshOrderEnum)
    {
    //case kMeshOrderLeftToRight:
    //case kMeshOrderRightToLeft:
        ...
        //meshLayout.LL_Pos = glm::vec4(-1.0, -1.0, 0.0f, 0.0f);
        //meshLayout.LR_Pos = glm::vec4(1.0, -1.0, 0.0f, 0.0f);
        //meshLayout.UL_Pos = glm::vec4(-1.0, 1.0, 0.0f, 0.0f);
        //meshLayout.UR_Pos = glm::vec4(1.0, 1.0, 0.0f, 0.0f);
        ...
        lensScaleOffset = glm::vec4(gLensScaleX, gLensScaleY, gLensOffsetLeftX, gLensOffsetLeftY);

        meshScaleOffset = glm::vec4(0.5f, 1.0f, -0.5f, 0.0f);

        // barrel mesh
        InitializeWarpMesh(pWarpData->warpMeshes[kMeshLeft], 
                           kMeshLeft, 
                           gWarpMeshRows, 
                           gWarpMeshCols, 
                           meshLayout, 
                           ...
                           lensScaleOffset);
#ifdef ENABLE_CAMERA
        if(gSvrCameraManager.IsSinglePass())
        {
            InitializeWarpMesh(pWarpData->warpMeshes[kMeshLeft_Camera],
                           kMeshLeft_Camera,
                           gWarpMeshRows,
                           gWarpMeshCols,
                           meshLayout,
...
#endif
        // no-distortion mesh
        InitializeWarpMesh(pWarpData->warpMeshes[kMeshLeft_Low], 
                           kMeshLeft_Low, 
                           undistortedRows, 
                           undistortedCols, 
                           meshLayout, 
        ...
        InitializeWarpMesh(pWarpData->warpMeshes[kMeshLeft_Sphere], 
                           kMeshLeft_Sphere, 
                           gWarpMeshRows, 
                           gWarpMeshCols, 
                           meshLayout, 
        ...
        lensScaleOffset = glm::vec4(gLensScaleX, gLensScaleY, gLensOffsetRightX, gLensOffsetRightY);

        meshScaleOffset = glm::vec4(0.5f, 1.0f, 0.5f, 0.0f);

        // barrel mesh
        InitializeWarpMesh(pWarpData->warpMeshes[kMeshRight], 
                           kMeshRight, 
                           gWarpMeshRows, 
                           gWarpMeshCols, 
                           meshLayout, 
        ...
        if(gSvrCameraManager.IsSinglePass())
        {
            InitializeWarpMesh(pWarpData->warpMeshes[kMeshRight_Camera],
                           kMeshRight_Camera,
                           gWarpMeshRows,
                           gWarpMeshCols,
                           meshLayout,
...
#endif
        // no-distortion mesh
        InitializeWarpMesh(pWarpData->warpMeshes[kMeshRight_Low], 
                           kMeshRight_Low, 
                           undistortedRows, 
                           undistortedCols, 
                           meshLayout, 
        ...
        // sphere mesh
        meshScaleOffset = glm::vec4(1.0f, 1.0f, 0.0f, 0.0f);
        InitializeWarpMesh(pWarpData->warpMeshes[kMeshRight_Sphere], 
                           kMeshRight_Sphere, 
                           gWarpMeshRows, 
                           gWarpMeshCols, 
                           meshLayout, 
        ...
        //
        //    // barrel mesh
        //    InitializeWarpMesh(pWarpData->warpMeshes[kMeshUL],
        //                       kMeshUL,
        //                       gWarpMeshRows,
        //                       gWarpMeshCols,
        //                       meshLayout,
        ...
        //
        //    // no-distortion mesh
        //    InitializeWarpMesh(pWarpData->warpMeshes[kMeshUL_Low],
        //                       kMeshUL_Low,
        //                       undistortedRows,
        //                       undistortedCols,
        //                       meshLayout,
        ...
        //    // sphere mesh
        //    meshScaleOffset = glm::vec4(1.0f, 1.0f, 0.0f, 0.0f);
        //    InitializeWarpMesh(pWarpData->warpMeshes[kMeshUL_Sphere],
        //                       kMeshUL_Sphere,
        //                       gWarpMeshRows,
        //                       gWarpMeshCols,
        //                       meshLayout,
        ...
        //    // barrel mesh
        //    InitializeWarpMesh(pWarpData->warpMeshes[kMeshUR],
        //                       kMeshUR,
        //                       gWarpMeshRows,
        //                       gWarpMeshCols,
        //                       meshLayout,
        ...
        //
        //    // no-distortion mesh
        //    InitializeWarpMesh(pWarpData->warpMeshes[kMeshUR_Low],
        //                       kMeshUR_Low,
        //                       undistortedRows,
        //                       undistortedCols,
        //                       meshLayout,
        ...
        //    // sphere mesh
        //    meshScaleOffset = glm::vec4(1.0f, 1.0f, 0.0f, 0.0f);
        //    InitializeWarpMesh(pWarpData->warpMeshes[kMeshUR_Sphere],
        //                       kMeshUR_Sphere,
        //                       gWarpMeshRows,
        //                       gWarpMeshCols,
        //                       meshLayout,
        ...
        //    // barrel mesh
        //    InitializeWarpMesh(pWarpData->warpMeshes[kMeshLL],
        //                       kMeshLL,
        //                       gWarpMeshRows,
        //                       gWarpMeshCols,
        //                       meshLayout,
        ...
        //    // no-distortion mesh
        //    InitializeWarpMesh(pWarpData->warpMeshes[kMeshLL_Low],
        //                       kMeshLL_Low,
        //                       undistortedRows,
        //                       undistortedCols,
        //                       meshLayout,
        ...
        //    // sphere mesh
        //    meshScaleOffset = glm::vec4(1.0f, 1.0f, 0.0f, 0.0f);
        //    InitializeWarpMesh(pWarpData->warpMeshes[kMeshLL_Sphere],
        //                       kMeshLL_Sphere,
        //                       gWarpMeshRows,
        //                       gWarpMeshCols,
        //                       meshLayout,
        ...
        //    // barrel mesh
        //    InitializeWarpMesh(pWarpData->warpMeshes[kMeshLR],
        //                       kMeshLR,
        //                       gWarpMeshRows,
        //                       gWarpMeshCols,
        //                       meshLayout,
        ...
        //
        //    // no-distortion mesh
        //    InitializeWarpMesh(pWarpData->warpMeshes[kMeshLR_Low],
        //                       kMeshLR_Low,
        //                       undistortedRows,
        //                       undistortedCols,
        //                       meshLayout,
        ...
        //    // sphere mesh
        //    meshScaleOffset = glm::vec4(1.0f, 1.0f, 0.0f, 0.0f);
        //    InitializeWarpMesh(pWarpData->warpMeshes[kMeshLR_Sphere],
        //                       kMeshLR_Sphere,
        //                       gWarpMeshRows,
        //                       gWarpMeshCols,
        //                       meshLayout,
    ...
    lensScaleOffset = glm::vec4(1.0f, 1.0f, 0.0f, 0.0f);

    InitializeWarpMesh(pWarpData->warpMeshes[kMeshOverlay], kMeshOverlay, gLayerMeshRows, gLayerMeshCols, meshLayout, meshScaleOffset, meshMode, kBarrelDistortion, lensScaleOffset);
    ...
    for (uint32_t first = 0; first < numShadersInMap; first++)
    {
        for (uint32_t second = first + 1; second < numShadersInMap; second++)
        {
            ...
            {
                LOGE("ERROR! Shader map %d and %d have the same mask (0x%08x)!", first, second, tempShaderMap[first]);
            }
        }
    }
    ...
    pWarpData->numWarpShaders *= 2;  // For the Subsampled Handling ones
    ...
    for (uint32_t whichShader = 0; whichShader < numBaseShaders; whichShader++)
    {
        ...
        pWarpData->pWarpShaderMap[whichShader + numBaseShaders] = pWarpData->pWarpShaderMap[whichShader] | kWarpProjected;
    }

    for (uint32_t whichShader = 0; whichShader < 2 * numBaseShaders; whichShader++)
    {
        // Third set is above plus UV handling
        pWarpData->pWarpShaderMap[whichShader + 2 * numBaseShaders] = pWarpData->pWarpShaderMap[whichShader] | kWarpUvScaleOffset | kWarpUvClamp;
    }

    for (uint32_t whichShader = 0; whichShader < 4 * numBaseShaders; whichShader++)
    {
        // Forth set is above plus Array handling
        pWarpData->pWarpShaderMap[whichShader + 4 * numBaseShaders] = pWarpData->pWarpShaderMap[whichShader] | kWarpArray;
    }

    for (uint32_t whichShader = 0; whichShader < 8 * numBaseShaders; whichShader++)
    {
        // Fifth set is above plus Subsampled handling
        pWarpData->pWarpShaderMap[whichShader + 8 * numBaseShaders] = pWarpData->pWarpShaderMap[whichShader] | kWarpSubsampled;
    }
    ...
    for (uint32_t first = 0; first < pWarpData->numWarpShaders; first++)
    {
        ...
        for (uint32_t second = first + 1; second < pWarpData->numWarpShaders; second++)
        {
            ...
            {
                LOGE("ERROR! Shader map %d and %d have the same mask (0x%08x)!", first, second, pWarpData->pWarpShaderMap[first]);
            }
        }
    }
    ...
    {   // Blit Shader
        ...
        if (!pWarpData->blitShader.Initialize(numVertStrings, vertShaderStrings, numFragStrings, fragShaderStrings, "svrBlitQuadVs", "svrBlitQuadFs"))
        {
    ...
    {   // Blit YUV Shader
        ...
        if (!pWarpData->blitYuvShader.Initialize(numVertStrings, vertShaderStrings, numFragStrings, fragShaderStrings, "svrBlitQuadVs", "svrBlitQuadYuvFs"))
        {
    ...
    {   // Blit Image Shader
        ...
        if (!pWarpData->blitImageShader.Initialize(numVertStrings, vertShaderStrings, numFragStrings, fragShaderStrings, "svrBlitQuadVs_Image", "svrBlitQuadFs_Image"))
        {
    ...
    {   // Blit YUV Image Shader
        ...
        if (!pWarpData->blitYuvImageShader.Initialize(numVertStrings, vertShaderStrings, numFragStrings, fragShaderStrings, "svrBlitQuadVs_Image", "svrBlitQuadYuvFs_Image"))
        {
...
#ifdef ENABLE_REMOTE_XR_RENDERING
    if(gEnableRVR) {
        localWidth = gAppContext->deviceInfo.targetEyeWidthPixels;
        localHeight = gAppContext->deviceInfo.targetEyeHeightPixels;
        LOGI("New width height ( %dx%d )", localWidth, localHeight);
    }
#endif

    for (int whichEye = 0; whichEye < kNumEyes; whichEye++)
    {
        LOGI("Creating eye render target ( %dx%d ), isProtectedContent: %d", localWidth, localHeight, gAppContext->modeContext->isProtectedContent);
        pWarpData->eyeTarget[whichEye].Initialize(localWidth, localHeight, 1, GL_RGBA8, false, gAppContext->modeContext->isProtectedContent);

        LOGI("Creating overlay render target ( %dx%d )", localWidth, localHeight);
        pWarpData->overlayTarget[whichEye].Initialize(localWidth, localHeight, 1, GL_RGBA8, false, gAppContext->modeContext->isProtectedContent);
    }

    // Create the sampler object for reprojected samplers
    float borderColor[] = { gClampBorderColorR, gClampBorderColorG, gClampBorderColorB, gClampBorderColorA };
    ...
    {
        GL(glSamplerParameterfv(pWarpData->eyeSamplerObj, TEXTURE_BORDER_COLOR_EXT, &borderColor[0]));
    ...
    {
        GL(glSamplerParameterfv(pWarpData->overlaySamplerObj, TEXTURE_BORDER_COLOR_EXT, &borderColor[0]));
        GL(glSamplerParameteri(pWarpData->overlaySamplerObj, GL_TEXTURE_WRAP_S, CLAMP_TO_BORDER_EXT));
        GL(glSamplerParameteri(pWarpData->overlaySamplerObj, GL_TEXTURE_WRAP_T, CLAMP_TO_BORDER_EXT));
    }
...
{
#ifdef ENABLE_CAMERA
    if (gEnableCameraLayer == true)
    {
        gSvrCameraManager.Shutdown();
    }
#endif

    if (gUseDistortionMeshData)
    {
        gSxrDistortionMesh.Finalize();
    }
    ...
    LOGI("Destroying occlude assets...");
    pWarpData->occludeMeshGeom.Destroy();
    pWarpData->occludeMeshShader.Destroy();
    pWarpData->occludeMeshArrayShader.Destroy();
...
//-----------------------------------------------------------------------------
unsigned int GetVulkanInteropHandle(sxrRenderLayer *pRenderLayer)
//-----------------------------------------------------------------------------
{
    ...
    GL(glImportMemoryFdKHR(oneEntry.hMemory, oneEntry.memSize, HANDLE_TYPE_OPAQUE_FD_KHR, pRenderLayer->imageHandle));
    ...
    GL(glTexStorageMem2DKHR(GL_TEXTURE_2D, oneEntry.numMips, internalFormat, oneEntry.width, oneEntry.height, oneEntry.hMemory, 0));

    // No longer need a bound texture
    GL(glBindTexture(GL_TEXTURE_2D, 0));

    // Finally, add this one to the map
    LOGI("Adding Vulkan texture (%d -> %d) to the list", pRenderLayer->imageHandle, oneEntry.hTexture);
...
//-----------------------------------------------------------------------------
void L_MakeNdcCoords(sxrLayoutCoords &ndcCoords, sxrLayoutCoords &srcCoords)
//-----------------------------------------------------------------------------
{
    ndcCoords = srcCoords;
...
//-----------------------------------------------------------------------------
{
    ...
    // Compositing value
    if (ndcCoords.LowerLeftPos[0] < -gCompositeRadius || ndcCoords.LowerLeftPos[1] < -gCompositeRadius ||
        ndcCoords.UpperLeftPos[0] < -gCompositeRadius || ndcCoords.UpperLeftPos[1] >  gCompositeRadius ||
        ndcCoords.LowerRightPos[0] > gCompositeRadius || ndcCoords.LowerRightPos[1] < -gCompositeRadius ||
        ndcCoords.UpperRightPos[0] > gCompositeRadius || ndcCoords.UpperRightPos[1] > gCompositeRadius)
    {
...
//-----------------------------------------------------------------------------
void L_UpdateCompositeBuffer(Svr::SvrRenderTarget &whichTarget, svrFrameParamsInternal *pWarpFrame, uint32_t startLayer, sxrEyeMask eyeMask, uint32_t layerFlags)
//-----------------------------------------------------------------------------
{
    // Using gWarpData (SvrAsyncWarpResources)
    SvrAsyncWarpResources* pWarpData = &gWarpData;

    if (gLogEyeRender) LOGI("        Updating Compositing buffer 0x%08x...", whichTarget.GetColorAttachment());
    if (gLogEyeRender) LOGI("            Start Layer = %d; Eye Mask = %d; Layer flag = 0x%08x", startLayer, eyeMask, layerFlags);
    whichTarget.Bind();
    ...
    for (uint32_t whichLayer = startLayer; whichLayer < SXR_MAX_RENDER_LAYERS; whichLayer++)
    {
        if (pWarpFrame->frameParams.renderLayers[whichLayer].imageHandle != 0)
        {
            if (gLogEyeRender) LOGI("        Layer %d: Eye Mask = %d; Layer flag = 0x%08x", whichLayer, pWarpFrame->frameParams.renderLayers[whichLayer].eyeMask, pWarpFrame->frameParams.renderLayers[whichLayer].layerFlags);
        }
        ...
        {
            if (gLogEyeRender) LOGI("        Skipping Layer %d: Layer flag (0x%08x) mismatch (Mask = 0x%08x)", whichLayer, pWarpFrame->frameParams.renderLayers[whichLayer].layerFlags, layerFlags);
            continue;
        }
#ifdef ENABLE_REMOTE_XR_RENDERING
        }
        ...
        {
            if (gLogEyeRender) LOGI("        Skipping Layer %d: Image type disqualifies", whichLayer);
            continue;
        }
    ...
    whichTarget.Unbind();
    if (gLogEyeRender) LOGI("        Compositing buffer updated (0x%08x)...", whichTarget.GetColorAttachment());
}
...
//-----------------------------------------------------------------------------
{
    ...
    case kIgnore:
        ...
        break;
    case kEntire:
        GL( glScissor(0, 0, gAppContext->modeContext->warpRenderSurfaceWidth, gAppContext->modeContext->warpRenderSurfaceHeight) );
        // LOGI("Scissor: (%d, %d, %d, %d)", 0, 0, gAppContext->modeContext->warpRenderSurfaceWidth, gAppContext->modeContext->warpRenderSurfaceHeight);
        break;
    case kLeftHalf:
        GL( glScissor(0, 0, gAppContext->modeContext->warpRenderSurfaceWidth / 2, gAppContext->modeContext->warpRenderSurfaceHeight) );
        ...
        break;
    case kRightHalf:
        GL( glScissor(gAppContext->modeContext->warpRenderSurfaceWidth / 2, 0, gAppContext->modeContext->warpRenderSurfaceWidth / 2, gAppContext->modeContext->warpRenderSurfaceHeight) );
        ...
        break;
    case kTopHalf:
        GL( glScissor(0, gAppContext->modeContext->warpRenderSurfaceHeight / 2, gAppContext->modeContext->warpRenderSurfaceWidth, gAppContext->modeContext->warpRenderSurfaceHeight / 2) );
        ...
        break;
    case kBottomHalf:
        GL( glScissor(0, 0, gAppContext->modeContext->warpRenderSurfaceWidth, gAppContext->modeContext->warpRenderSurfaceHeight / 2) );
        ...
        break;
    case kLowerLeft:
        GL(glScissor(0, 0, gAppContext->modeContext->warpRenderSurfaceWidth / 2, gAppContext->modeContext->warpRenderSurfaceHeight / 2));
        // LOGI("Scissor: (%d, %d, %d, %d)", 0, 0, gAppContext->modeContext->warpRenderSurfaceWidth / 2, gAppContext->modeContext->warpRenderSurfaceHeight / 2);
        break;
    case kUpperLeft:
        GL(glScissor(0, gAppContext->modeContext->warpRenderSurfaceHeight / 2, gAppContext->modeContext->warpRenderSurfaceWidth / 2, gAppContext->modeContext->warpRenderSurfaceHeight));
        // LOGI("Scissor: (%d, %d, %d, %d)", 0, gAppContext->modeContext->warpRenderSurfaceHeight / 2, gAppContext->modeContext->warpRenderSurfaceWidth / 2, gAppContext->modeContext->warpRenderSurfaceHeight);
        break;
    case kLowerRight:
        GL(glScissor(gAppContext->modeContext->warpRenderSurfaceWidth / 2, 0, gAppContext->modeContext->warpRenderSurfaceWidth, gAppContext->modeContext->warpRenderSurfaceHeight / 2));
        // LOGI("Scissor: (%d, %d, %d, %d)", gAppContext->modeContext->warpRenderSurfaceWidth / 2, 0, gAppContext->modeContext->warpRenderSurfaceWidth, gAppContext->modeContext->warpRenderSurfaceHeight / 2);
        break;
    case kUpperRight:
        GL(glScissor(gAppContext->modeContext->warpRenderSurfaceWidth / 2, gAppContext->modeContext->warpRenderSurfaceHeight / 2, gAppContext->modeContext->warpRenderSurfaceWidth, gAppContext->modeContext->warpRenderSurfaceHeight));
        // LOGI("Scissor: (%d, %d, %d, %d)", gAppContext->modeContext->warpRenderSurfaceWidth / 2, gAppContext->modeContext->warpRenderSurfaceHeight / 2, gAppContext->modeContext->warpRenderSurfaceWidth, gAppContext->modeContext->warpRenderSurfaceHeight);
        break;
    }
}
...
//-----------------------------------------------------------------------------
{
    ...
    {
    case kEntire:
        GL( glStartTilingQCOM(0, 0, gAppContext->modeContext->warpRenderSurfaceWidth, gAppContext->modeContext->warpRenderSurfaceHeight, 0 ) );
        break;
    case kLeftHalf:
        GL( glStartTilingQCOM(0, 0, gAppContext->modeContext->warpRenderSurfaceWidth / 2, gAppContext->modeContext->warpRenderSurfaceHeight, 0) );
        break;
    case kRightHalf:
        GL( glStartTilingQCOM(gAppContext->modeContext->warpRenderSurfaceWidth / 2, 0, gAppContext->modeContext->warpRenderSurfaceWidth / 2, gAppContext->modeContext->warpRenderSurfaceHeight, 0) );
        break;
    case kTopHalf:
        GL( glStartTilingQCOM(0, gAppContext->modeContext->warpRenderSurfaceHeight / 2, gAppContext->modeContext->warpRenderSurfaceWidth, gAppContext->modeContext->warpRenderSurfaceHeight / 2, 0) );
        break;
    case kBottomHalf:
        GL( glStartTilingQCOM(0, 0, gAppContext->modeContext->warpRenderSurfaceWidth, gAppContext->modeContext->warpRenderSurfaceHeight / 2, 0) );
        break;
    default:
        break;
    }
...
//-----------------------------------------------------------------------------
{
    for (int whichLayer = 0; whichLayer < SXR_MAX_RENDER_LAYERS; whichLayer++)
    {
        if ( (pWarpFrame->frameParams.renderLayers[whichLayer].imageType == kTypeEquiRectTexture) ||
             (pWarpFrame->frameParams.renderLayers[whichLayer].imageType == kTypeEquiRectImage)   ||
             (pWarpFrame->frameParams.renderLayers[whichLayer].imageType == kTypeCubemapTexture) )
...
struct ShaderParamStruct
{
    ...
    glm::mat4           texWarpMatrix[3];         // Location = 2
...
//-----------------------------------------------------------------------------
void L_SetShaderUniforms(svrFrameParamsInternal* pWarpFrame, ShaderParamStruct *pParams, unsigned int flags, unsigned int eyeBuffer, unsigned int samplerType, unsigned int samplerObj, sxrEyeMask eyeMask)
//-----------------------------------------------------------------------------
{
    if (gLogShaderUniforms)
        LOGI("********** Setting Shader Parameters (Shader Handle = %d) **********", pParams->pShader->GetShaderId());

    if (gLogShaderUniforms) LOGI("Setting posScaleOffset: (%0.2f, %0.2f, %0.2f, %0.2f)", pParams->posScaleoffset.x, pParams->posScaleoffset.y, pParams->posScaleoffset.z, pParams->posScaleoffset.w);
    pParams->pShader->SetUniformVec4(1, pParams->posScaleoffset);

    if (gLogShaderUniforms) LOGI("Setting skewSquashMatrix: (%0.2f, %0.2f, %0.2f, %0.2f)", pParams->skewSquashMatrix[0][0], pParams->skewSquashMatrix[0][1], pParams->skewSquashMatrix[1][0], pParams->skewSquashMatrix[1][1]);
    ...
    if (flags & kEnableTexWarpMatrix)
    {
        ...
        {
            // Always log "green"
            glm::mat4 L = pParams->texWarpMatrix[1];
        ...
        if (gSupportColorPlaneWarp)
        {
            pParams->pShader->SetUniformMat4fv(2, 3, &pParams->texWarpMatrix[0][0][0]);
        }
        else
        {
            // Since no planes, use "green"
            pParams->pShader->SetUniformMat4(2, pParams->texWarpMatrix[1]);
        }
    }

    if (flags & kEnableEyeBorder)
    {
        if (gLogShaderUniforms) LOGI("Setting eyeBorder: (%0.2f, %0.2f, %0.2f, %0.2f)", pParams->eyeBorder.x, pParams->eyeBorder.y, pParams->eyeBorder.z, pParams->eyeBorder.w);
        pParams->pShader->SetUniformVec4(3, pParams->eyeBorder);
    } 

    if (flags & kEnableUvOffset)
    {
        if (gLogShaderUniforms) LOGI("Setting uvScaleOffset: (%0.2f, %0.2f, %0.2f, %0.2f)", pParams->uvScaleoffset.x, pParams->uvScaleoffset.y, pParams->uvScaleoffset.z, pParams->uvScaleoffset.w);
    ...
    {
        if (gLogShaderUniforms) LOGI("Setting vignetteParams: (%0.2f, %0.2f, %0.2f, %0.2f)", pParams->vignetteParams.x, pParams->vignetteParams.y, pParams->vignetteParams.z, pParams->vignetteParams.w);
    ...
    {
        if (gLogShaderUniforms) LOGI("Setting arrayLayer: (%0.2f, %0.2f, %0.2f, %0.2f)", pParams->arrayLayer.x, pParams->arrayLayer.y, pParams->arrayLayer.z, pParams->arrayLayer.w);
    ...
    if (eyeBuffer != 0)
    {
        ...
        QVRCAMERA_FRAME_FORMAT cameraFormat = gSvrCameraManager.GetCameraFrameFormat();
        if (cameraFormat == QVRCAMERA_FRAME_FORMAT_RAW16_MONO && gSvrCameraManager.IsMonoFloat16() == false)
        {
            // 16 bit integer
            GL(glSamplerParameteri(gWarpData.eyeSamplerObj, GL_TEXTURE_MIN_FILTER, GL_NEAREST));
            GL(glSamplerParameteri(gWarpData.eyeSamplerObj, GL_TEXTURE_MAG_FILTER, GL_NEAREST));
        }
        else
        {
            GL(glSamplerParameteri(gWarpData.eyeSamplerObj, GL_TEXTURE_MIN_FILTER, GL_LINEAR));
            GL(glSamplerParameteri(gWarpData.eyeSamplerObj, GL_TEXTURE_MAG_FILTER, GL_LINEAR));
        }
    ...
    if (gEnableMotionVectors)
    {
        ...
        GLenum syncResult = GL(glClientWaitSync(pWarpFrame->motionVectorSync, GL_SYNC_FLUSH_COMMANDS_BIT, 0));
        if (gFrameDoubled && gUseMotionVectors && ((syncResult == GL_ALREADY_SIGNALED) || (syncResult == GL_CONDITION_SATISFIED)))
        {
...
void L_SelectOverrideMesh(unsigned int &mesh1A, unsigned int &mesh1B, unsigned int &mesh2A, unsigned int &mesh2B)
{
    ...
    for (int whichMesh = 0; whichMesh < 4; whichMesh++)
    {
        ...
        switch (whichMesh)
        {
        case 0: pOneMesh = &mesh1A;   break;
        case 1: pOneMesh = &mesh1B;   break;
        case 2: pOneMesh = &mesh2A;   break;
        case 3: pOneMesh = &mesh2B;   break;
        }
    ...
    for (int i = 0; i < NUM_SWAP_FRAMES && pReturnFrame == NULL; i++)
    {
        ...
        if (pCheckFrame->frameParams.renderLayers[0].vulkanInfo.renderSemaphore != 0)
        {
            ...
            if (pCheckFrame->externalSync == 0)
            {
                ...
                {
                    eglDestroySyncKHR(gVkConsumer_eglDisplay, eglSyncList[checkFrameCount % NUM_SWAP_FRAMES]);
                ...
                EGLint vSyncAttribs[] =
                {
                    EGL_SYNC_NATIVE_FENCE_FD_ANDROID, (EGLint)pCheckFrame->frameParams.renderLayers[0].vulkanInfo.renderSemaphore,
                    EGL_NONE,
                };

                // LOGI("Getting sync handle from external fd (%d)...", (int)pCheckFrame->frameParams.renderLayers[0].vulkanInfo.renderSemaphore);
                
                EGLSyncKHR externalSync = eglCreateSyncKHR(gVkConsumer_eglDisplay, EGL_SYNC_NATIVE_FENCE_ANDROID, vSyncAttribs);
                LOG_EGL_RESOURCE("%s:%i:threadid=%i, eglCreateSyncKHR(gVkConsumer_eglDisplay=%p=externalSync=%p)", __FILE__, __LINE__, gettid(), gVkConsumer_eglDisplay, externalSync);
            ...
            if (eglResult == EGL_TIMEOUT_EXPIRED_KHR)
            {
                ...
                PROFILE_ENTER(GROUP_TIMEWARP, 0, "Vulkan Frame %d NOT finished on GPU", pCheckFrame->frameParams.frameIndex);
            ...
            if (eglResult != EGL_CONDITION_SATISFIED_KHR)
            {
                ...
                PROFILE_ENTER(GROUP_TIMEWARP, 0, "Vulkan Frame %d NOT finished on GPU (Failure)", pCheckFrame->frameParams.frameIndex);
                PROFILE_EXIT(GROUP_TIMEWARP);
                continue;
            }

            PROFILE_ENTER(GROUP_TIMEWARP, 0, "Vulkan Frame %d finished on GPU", pCheckFrame->frameParams.frameIndex);
            PROFILE_EXIT(GROUP_TIMEWARP);
            ...
            EGLBoolean eglResult = EGL(eglGetSyncAttribKHR(gVkConsumer_eglDisplay, (EGLSyncKHR)pCheckFrame->externalSync, EGL_SYNC_STATUS_KHR, &syncStatus));
            if (syncStatus == EGL_UNSIGNALED_KHR)
            {
                // This frame has not finished
                //LOGI("TIMEWARP: GPU not finished with frame %d", checkFrameCount);
                PROFILE_ENTER(GROUP_TIMEWARP, 0, "Vulkan Frame %d (Slot %d) NOT finished on GPU", pCheckFrame->frameParams.frameIndex, checkFrameCount % NUM_SWAP_FRAMES);
                PROFILE_EXIT(GROUP_TIMEWARP);
                continue;
            }
            ...
            if (eglResult != EGL_TRUE)
            {
                ...
                PROFILE_ENTER(GROUP_TIMEWARP, 0, "Vulkan Frame %d (Slot %d) NOT finished on GPU (Failure)", pCheckFrame->frameParams.frameIndex, checkFrameCount % NUM_SWAP_FRAMES);
                PROFILE_EXIT(GROUP_TIMEWARP);
                continue;
            }

            PROFILE_ENTER(GROUP_TIMEWARP, 0, "Vulkan Frame %d (Slot %d) finished on GPU", pCheckFrame->frameParams.frameIndex, checkFrameCount % NUM_SWAP_FRAMES);
            PROFILE_EXIT(GROUP_TIMEWARP);
#endif
        }
        else
        {
            ...
            GLenum syncResult = GL(glClientWaitSync(pCheckFrame->frameSync, GL_SYNC_FLUSH_COMMANDS_BIT, 0));
            switch (syncResult)
            {
            case GL_ALREADY_SIGNALED:       // indicates that sync was signaled at the time that glClientWaitSync was called.
                ...
                PROFILE_ENTER(GROUP_TIMEWARP, 0, "Frame %d (Slot %d) finished on GPU", pCheckFrame->frameParams.frameIndex, checkFrameCount % NUM_SWAP_FRAMES);
                PROFILE_EXIT(GROUP_TIMEWARP);
                break;
            ...
            case GL_TIMEOUT_EXPIRED:        // indicates that at least timeout nanoseconds passed and sync did not become signaled.
                ...
                PROFILE_ENTER(GROUP_TIMEWARP, 0, "Frame %d (Slot %d) NOT finished on GPU", pCheckFrame->frameParams.frameIndex, checkFrameCount % NUM_SWAP_FRAMES);
                PROFILE_EXIT(GROUP_TIMEWARP);
                continue;
            ...
            case GL_CONDITION_SATISFIED:    // indicates that sync was signaled before the timeout expired.
                ...
                LOGE("Frame %d returned GL_CONDITION_SATISFIED for glClientWaitSync", checkFrameCount % NUM_SWAP_FRAMES);
                PROFILE_ENTER(GROUP_TIMEWARP, 0, "Frame %d (Slot %d) finished on GPU", pCheckFrame->frameParams.frameIndex, checkFrameCount % NUM_SWAP_FRAMES);
                PROFILE_EXIT(GROUP_TIMEWARP);
                break;

            case GL_WAIT_FAILED:            // indicates that an error occurred. Additionally, an OpenGL error will be generated.
                LOGE("Frame %d returned GL_WAIT_FAILED for glClientWaitSync", checkFrameCount % NUM_SWAP_FRAMES);
                PROFILE_ENTER(GROUP_TIMEWARP, 0, "Frame %d (Slot %d) NOT finished on GPU (Failure)", pCheckFrame->frameParams.frameIndex, checkFrameCount % NUM_SWAP_FRAMES);
                PROFILE_EXIT(GROUP_TIMEWARP);
                continue;
            }
        }
...
//-----------------------------------------------------------------------------
bool L_IsUvIdentity(sxrRenderLayer &renderLayer)
//-----------------------------------------------------------------------------
{
...
//-----------------------------------------------------------------------------
bool L_NeedUvScaleOffset(sxrRenderLayer &renderLayer, glm::vec4 &uvScaleoffset, glm::vec4 &eyeBorder)
//-----------------------------------------------------------------------------
{
...
//-----------------------------------------------------------------------------
Svr::SvrShader *L_GetLayerShader(svrFrameParamsInternal* pWarpFrame, uint32_t whichLayer, sxrEyeMask eyeMask, unsigned int &paramFlag, ShaderParamStruct &shaderParams, unsigned int &samplerType)
//-----------------------------------------------------------------------------
{
    // Using gWarpData (SvrAsyncWarpResources)
    SvrAsyncWarpResources* pWarpData = &gWarpData;
    ...
    if (L_NeedUvScaleOffset(pWarpFrame->frameParams.renderLayers[whichLayer], shaderParams.uvScaleoffset, shaderParams.eyeBorder))
    {
    ...
    if (gMeshVignette)
    {
        ...
        shaderParams.vignetteParams = glm::vec4(gVignetteRadius, gLensPolyUnitRadius, gLensPolyUnitRadius - gVignetteRadius, 0.0f);
    }
        ...
        pWarpFrame->frameParams.renderLayers[whichLayer].imageType == kTypeEquiRectImage)
    {
        ...
        glm::fquat poseQuat = glm::fquat(poseState.pose.rotation.w, poseState.pose.rotation.x, poseState.pose.rotation.y, poseState.pose.rotation.z);
        glm::mat4 viewMat = glm::mat4_cast(poseQuat);
        glm::mat4 modelMat = inverse(viewMat);  // Need the inverse of the pose matrix to be used as a model matrix
        shaderParams.modelMatrix = modelMat;

        // ...transform matrix...
        paramFlag |= kEnableTransform;
        glm::mat4 transformMat = glm::make_mat4(pWarpFrame->frameParams.renderLayers[whichLayer].imageCoords.TransformMatrix);
    ...
    // kWarpSubsampled = (1 << 9),
    if (pWarpFrame->frameParams.renderLayers[whichLayer].layerFlags & kLayerFlagSubsampled)
    {
        shaderMask |= kWarpSubsampled;
    }

#ifdef ENABLE_CAMERA
    // kTypeCamera        = (1 << 10),
    if(pWarpFrame->frameParams.renderLayers[whichLayer].imageType == kTypeCamera &&
       gSvrCameraManager.IsSinglePass() == true)
    {
        shaderMask |= kWarpCameraImage;

        /*
         * Based on camera image type, the sampler type will change.
         */
        QVRCAMERA_FRAME_FORMAT cameraFormat = gSvrCameraManager.GetCameraFrameFormat();
        if (cameraFormat == QVRCAMERA_FRAME_FORMAT_RAW16_MONO)
        {
...
//-----------------------------------------------------------------------------
bool L_NeedCustomMesh(sxrRenderLayer &renderLayer)
//-----------------------------------------------------------------------------
{
...
//-----------------------------------------------------------------------------
void L_BlitOneLayer(svrFrameParamsInternal *pWarpFrame, sxrEyeMask eyeMask, uint32_t whichLayer)
//-----------------------------------------------------------------------------
{
    ...
    sxrLayoutCoords *pImageCoords = &pWarpFrame->frameParams.renderLayers[whichLayer].imageCoords;
    ...
    //lensScaleoffset = glm::vec4(1.0f, 1.0f, 0.0f, 0.0f);
    UpdateOverlayGeometry(gWarpData.warpMeshes[kMeshOverlay], meshLayout, posScaleoffset, lensScaleOffset);
...
#ifdef ENABLE_CAMERA
//-----------------------------------------------------------------------------
SxrResult L_CameraUpdate(svrFrameParamsInternal* pWarpFrame)
//-----------------------------------------------------------------------------
{
    /*
     * Only call once per frame for all eyes
     */
    SxrResult status = gSvrCameraManager.Update(); // gets the latest camera frame pixels
    if (status != SXR_ERROR_NONE)
    {
        //return status;
    }

    /*
     * Afer Update(), due to any internal failures, a desired 1 pass render correction
     * method may have been set to 2 pass as a fallback. So get the pass flag and 
     * proceed accordingly.
     */
    bool cameraSinglePass = gSvrCameraManager.IsSinglePass(); 
    if (cameraSinglePass == false)
    {
        // Only call once per frame for all eyes
        status = gSvrCameraManager.Render();
    }
...
//-----------------------------------------------------------------------------
SxrResult L_ProcessCamera(svrFrameParamsInternal* pWarpFrame, uint32_t whichLayer)
//-----------------------------------------------------------------------------
{
    uint32_t cameraTextureID = 0;
    sxrEyeMask eyeMask = pWarpFrame->frameParams.renderLayers[whichLayer].eyeMask;
    ...
    if (cameraSinglePass == true)
    {
        /*
         * A single render mesh handles both camera lens & display lens correction.
         */
        cameraTextureID = gSvrCameraManager.GetUncorrectedTextureID(eyeMask);   
        if (gSvrCameraManager.IsStereoPair() == true)
        {
            /*
            if (eyeMask == kEyeMaskLeft)
            {
                // 4 UV coords for LEFT side of stereo image pair
                pWarpFrame->frameParams.renderLayers[whichLayer].imageCoords.LowerUVs[0] = 0.0f; //U lower left
                pWarpFrame->frameParams.renderLayers[whichLayer].imageCoords.LowerUVs[1] = 0.0f; //V lower left
                pWarpFrame->frameParams.renderLayers[whichLayer].imageCoords.LowerUVs[2] = 0.5f; //U lower right
                pWarpFrame->frameParams.renderLayers[whichLayer].imageCoords.LowerUVs[3] = 0.0f; //V lower right

                pWarpFrame->frameParams.renderLayers[whichLayer].imageCoords.UpperUVs[0] = 0.0f; // U upper left
                pWarpFrame->frameParams.renderLayers[whichLayer].imageCoords.UpperUVs[1] = 1.0f; // V upper left
                pWarpFrame->frameParams.renderLayers[whichLayer].imageCoords.UpperUVs[2] = 0.5f; // U upper right
                pWarpFrame->frameParams.renderLayers[whichLayer].imageCoords.UpperUVs[3] = 1.0f; // V upper right
            }
            else if (eyeMask == kEyeMaskRight)
            {
                // 4 UV coords for RIGHT side of stereo image pair
                pWarpFrame->frameParams.renderLayers[whichLayer].imageCoords.LowerUVs[0] = 0.5f; //U lower left
                pWarpFrame->frameParams.renderLayers[whichLayer].imageCoords.LowerUVs[1] = 0.0f; //V lower left
                pWarpFrame->frameParams.renderLayers[whichLayer].imageCoords.LowerUVs[2] = 1.0f; //U lower right
                pWarpFrame->frameParams.renderLayers[whichLayer].imageCoords.LowerUVs[3] = 0.0f; //V lower right

                pWarpFrame->frameParams.renderLayers[whichLayer].imageCoords.UpperUVs[0] = 0.5f; // U upper left
                pWarpFrame->frameParams.renderLayers[whichLayer].imageCoords.UpperUVs[1] = 1.0f; // V upper left
                pWarpFrame->frameParams.renderLayers[whichLayer].imageCoords.UpperUVs[2] = 1.0f; // U upper right
                pWarpFrame->frameParams.renderLayers[whichLayer].imageCoords.UpperUVs[3] = 1.0f; // V upper right
            }
            */
        }
    ...
    {
        /*
         * Two Pass - 2 render meshes are used. One internally by svrCameraManager to do
         * camera lens correction and one by this function's render to do display lens correction.
         */
        cameraTextureID = gSvrCameraManager.GetCorrectedTextureID(eyeMask);
    }

    /*
     * Common to both methods
     */
    pWarpFrame->frameParams.renderLayers[whichLayer].imageHandle = cameraTextureID;
...
//-----------------------------------------------------------------------------
void L_RenderOneEye(svrFrameParamsInternal* pWarpFrame, 
                    sxrEyeMask eyeMask, 
                    svrSurfaceSubset whichSurface, 
                    uint32_t whichMesh, 
                    uint32_t whichMeshSphere, 
                    uint32_t whichMeshCamera, 
                    glm::mat4 *warpMatrix, 
                    glm::mat4 *cameraWarpMatrix, 
                    glm::mat2 &skewSquashMatrix)
//-----------------------------------------------------------------------------
{
    // Verify Parameters
...
#ifdef ENABLE_REMOTE_XR_RENDERING
    if(gEnableRVR) {
        uint32_t whichEye = kLeftEye;
        if (eyeMask & kEyeMaskRight)
        {
            if (gLogEyeRender) LOGI("    [RVR] Compositing Right eye...");
            whichEye = kRightEye;
        }
        else
        {
            if (gLogEyeRender) LOGI("    [RVR] Compositing Left eye...");
            whichEye = kLeftEye;
        }
        GL(glDisable(GL_SCISSOR_TEST));
        L_UpdateCompositeBuffer(gWarpData.overlayTarget[whichEye], pWarpFrame, 0, eyeMask, kLayerFlagHeadLocked);
    ...
    for (uint32_t headLockLoop = 0; headLockLoop < 2; headLockLoop++)
    {
        ...
        for (uint32_t whichLayer = 0; whichLayer < SXR_MAX_RENDER_LAYERS; whichLayer++)
        {
            ...
            if (pWarpFrame->frameParams.renderLayers[whichLayer].imageHandle == 0 &&
                pWarpFrame->frameParams.renderLayers[whichLayer].imageType != kTypeCamera)
            {
            ...
            if (!(eyeMask & pWarpFrame->frameParams.renderLayers[whichLayer].eyeMask))
            {
                ...
                if (gLogEyeRender) LOGI("    Rendering Layer %d is NOT for this eye...", whichLayer);
                continue;
            }

            // If we are warped, do those on first loop.  Do head locked on second loop
            if (headLockLoop == 0 && (pWarpFrame->frameParams.renderLayers[whichLayer].layerFlags & kLayerFlagHeadLocked))
            {
                // This is warped loop but this is head locked layer
                if (gLogEyeRender) LOGI("    Rendering Layer %d is head locked so skipping this round...", whichLayer);
                continue;
            }

            if (headLockLoop == 1 && !(pWarpFrame->frameParams.renderLayers[whichLayer].layerFlags & kLayerFlagHeadLocked))
            {
                // This is head locked loop but this is warped layer
                if (gLogEyeRender) LOGI("    Rendering Layer %d is warped so skipping this round...", whichLayer);
                continue;
            }
            ...
            if (isComposite)
            {
                ...
                if (pWarpFrame->frameParams.renderLayers[whichLayer].layerFlags & kLayerFlagHeadLocked)
                {
                    ...
                    L_UpdateCompositeBuffer(gWarpData.overlayTarget[whichEye], pWarpFrame, whichLayer, eyeMask, pWarpFrame->frameParams.renderLayers[whichLayer].layerFlags);
                ...
                else
                {
                    ...
                    L_UpdateCompositeBuffer(gWarpData.eyeTarget[whichEye], pWarpFrame, whichLayer, eyeMask, pWarpFrame->frameParams.renderLayers[whichLayer].layerFlags);
            ...
            // Blending is set per layer
            if (!isComposite && (pWarpFrame->frameParams.renderLayers[whichLayer].layerFlags & kLayerFlagOpaque))
            {
                GL(glDisable(GL_BLEND));
            }
            else
            ...
            if (eyeMask & kEyeMaskLeft)
                shaderParams.posScaleoffset = glm::vec4(1.0f, 1.0f, gMeshOffsetLeftX, gMeshOffsetLeftY);
            else if (eyeMask & kEyeMaskRight)
                shaderParams.posScaleoffset = glm::vec4(1.0f, 1.0f, gMeshOffsetRightX, gMeshOffsetRightY);
            shaderParams.eyeBorder = glm::vec4(0.0f + onePixelWidth, 0.5f - onePixelHeight, 0.0f, 0.0f);
            shaderParams.uvScaleoffset = glm::vec4(1.0f, 1.0f, 0.0f, 0.0f);
             ...
             * Handle camera layer
             */
            bool useCameraWarpMatrix = false;
            if (pWarpFrame->frameParams.renderLayers[whichLayer].imageType == kTypeCamera)
            {
                if (gEnableCameraLayer == true && gSvrCameraManager.IsOK() == true)
                {
                    L_ProcessCamera(pWarpFrame, whichLayer);
                    // Camera layers need their own warp matrix
                    for (uint32_t whichPlane = 0; whichPlane < 3; whichPlane++)
                    {
                        shaderParams.texWarpMatrix[whichPlane] = cameraWarpMatrix[whichPlane];
                    }
                }
            }
            else
            {
                for (uint32_t whichPlane = 0; whichPlane < 3; whichPlane++)
                {
                    shaderParams.texWarpMatrix[whichPlane] = warpMatrix[whichPlane];
                }
            }
#else
            /*
            ...
            if (gEnableMotionVectors)
            {
                // Pass in the amount to adjust the space warp result texture
            ...
            else
            {
                ...
                pThisShader = L_GetLayerShader(pWarpFrame, whichLayer, eyeMask, paramFlag, shaderParams, samplerType);
            }

            if (pThisShader == NULL)
            {
                LOGE("Could not find a shader to handle render layer %d (Mask = 0x%08x)", whichLayer, 0xFFFFFFFF);
                continue;
            }
            ...
            if (pCurrentShader == NULL)
            {
                LOGE("Current shader to handle render layer %d is NULL (Mask = 0x%08x)", whichLayer, 0xFFFFFFFF);
                continue;
            }
            ...
            if (isComposite)
            {
                ...
                {
                    // This is overlay composite
                    imageHandle = gWarpData.overlayTarget[whichEye].GetColorAttachment();
                    if (gLogEyeRender) LOGI("    Composite overlay handle: 0x%08x", imageHandle);
                }
                else
                {
                    // This is eye layer composite
                    imageHandle = gWarpData.eyeTarget[whichEye].GetColorAttachment();
            ...
            if (gRenderMotionVectors && gpMotionData != NULL)
            {
                ...
                if (replacedEyeBuffer)
                {
                    ...
                    sxrLayoutCoords *pOneLayout = &pWarpFrame->frameParams.renderLayers[whichLayer].imageCoords;
            ...
            if (gWarpFrameCount > gSensorInitializeFrames)
            {
                ...
                if (!isComposite && L_NeedCustomMesh(pWarpFrame->frameParams.renderLayers[whichLayer]))
                {
                    ...
                    if (headLockLoop == 0)
                        L_SetShaderUniforms(pWarpFrame, &shaderParams, paramFlag, imageHandle, samplerType, gWarpData.eyeSamplerObj, eyeMask);
                    else
                        L_SetShaderUniforms(pWarpFrame, &shaderParams, paramFlag, imageHandle, samplerType, gWarpData.overlaySamplerObj, eyeMask);
                    ...
                    if (gUseDistortionMeshData)
                    {
                        SvrGeometry* mesh = gSxrDistortionMesh.UpdateAndGetQuadLayerMesh(ndcCoords, whichEye);
                        if (mesh && mesh->GetVbId() > 0)
                        {
                            //assert(mesh->GetVbId() > 0);
                            mesh->Submit(&gMeshAttribs[0], NUM_VERT_ATTRIBS);
                        }
                    }
                    else
                    {
                        svrMeshLayout meshLayout;
                        meshLayout.LL_Pos = glm::vec4(ndcCoords.LowerLeftPos[0], ndcCoords.LowerLeftPos[1], ndcCoords.LowerLeftPos[2], ndcCoords.LowerLeftPos[3]);
                        meshLayout.LR_Pos = glm::vec4(ndcCoords.LowerRightPos[0], ndcCoords.LowerRightPos[1], ndcCoords.LowerRightPos[2], ndcCoords.LowerRightPos[3]);
                        meshLayout.UL_Pos = glm::vec4(ndcCoords.UpperLeftPos[0], ndcCoords.UpperLeftPos[1], ndcCoords.UpperLeftPos[2], ndcCoords.UpperLeftPos[3]);
                        meshLayout.UR_Pos = glm::vec4(ndcCoords.UpperRightPos[0], ndcCoords.UpperRightPos[1], ndcCoords.UpperRightPos[2], ndcCoords.UpperRightPos[3]);
                        meshLayout.LL_UV = glm::vec2(ndcCoords.LowerUVs[0], ndcCoords.LowerUVs[1]);
                        meshLayout.LR_UV = glm::vec2(ndcCoords.LowerUVs[2], ndcCoords.LowerUVs[3]);
                        meshLayout.UL_UV = glm::vec2(ndcCoords.UpperUVs[0], ndcCoords.UpperUVs[1]);
                        meshLayout.UR_UV = glm::vec2(ndcCoords.UpperUVs[2], ndcCoords.UpperUVs[3]);
                ...
                else
                {
                    ...
                    if ((pWarpFrame->frameParams.renderLayers[whichLayer].imageType == kTypeEquiRectTexture) ||
                        ...
                        (pWarpFrame->frameParams.renderLayers[whichLayer].imageType == kTypeCubemapTexture))
                    {
                        gWarpData.warpMeshes[whichMeshSphere].Submit(&gMeshAttribs[0], NUM_VERT_ATTRIBS);
                    }
#if ENABLE_CAMERA
                    else if (pWarpFrame->frameParams.renderLayers[whichLayer].imageType == kTypeCamera &&
                             gSvrCameraManager.IsSinglePass() == true )
                    {
                        if (gSvrCameraManager.IsPaused() == false)
                        {
                            gWarpData.warpMeshes[whichMeshCamera].Submit(&gMeshAttribs[0], NUM_VERT_ATTRIBS);
                        }
                        else
                        {
                            // device is paused, clear to black. Race condition with camera, so not
...
//-----------------------------------------------------------------------------
{
    ...
    {
        LOGE("MilliSecondsToNextVSync(): Error! VSync time is in the future by %0.4f ms!", (float)(latestVsyncTimeNano - timestamp) * 0.000001f);
        return 0.0;
    }

    // How long ago was the last VSync?
    uint64_t timeSinceVsync = timestamp - latestVsyncTimeNano;
    if (timeSinceVsync > nanosPerVSync)
    {
        ...
        if(gLogPrediction)
            LOGE("Missing VSync time!  %0.4f milliseconds have passed but VSync interval is %0.4f!", timeSinceVsyncMs, msPerVSync);
...
//-----------------------------------------------------------------------------
{
    ...
    if (gUseQvrPerfModule && gAppContext->qvrHelper)
    {
        ...
        int32_t qRes = QVRServiceClient_SetThreadAttributesByType(gAppContext->qvrHelper, gAppContext->modeContext->warpThreadId, threadType);
        if (qRes != QVR_SUCCESS)
        {
    ...
    GL( glGetFramebufferAttachmentParameteriv(GL_READ_FRAMEBUFFER, GL_BACK, GL_FRAMEBUFFER_ATTACHMENT_COLOR_ENCODING, &nReadColorSpace) );
    ...
    float clearColor[4] = { gTimeWarpClearColorR, gTimeWarpClearColorG, gTimeWarpClearColorB, gTimeWarpClearColorA };
    ...
    while (true)
    {
...
#ifdef ENABLE_CAMERA
            || gSvrCameraManager.IsPaused() == true
#endif
            )
        {
            gWarpFrameCount = -100;
            LOGI("Frame not displayed since not in VR mode, or paused VR.");
        ...
        // Read and validate the vsync time
        if (gUseVSyncCallback || gUseLinePtr)
        {
            uint64_t qvrTimeStamp = 0;
            qvrservice_ts_t* qvrStamp(nullptr);
            int qRes = QVR_SUCCESS;
            if (gUseVSyncCallback)
            {
                qRes = QVRServiceClient_GetDisplayInterruptTimestamp(gAppContext->qvrHelper, DISP_INTERRUPT_VSYNC, &qvrStamp);
                if (qRes != QVR_SUCCESS)
                {
                    L_LogQvrError("QVRServiceClient_GetDisplayInterruptTimestamp(DISP_INTERRUPT_VSYNC)", qRes);
                }
            }
            else if (gUseLinePtr)
            {
                ...
                if (qRes != QVR_SUCCESS)
                {
                    L_LogQvrError("QVRServiceClient_GetDisplayInterruptTimestamp(DISP_INTERRUPT_LINEPTR)", qRes);
                }
            }
            ...
            {
                LOGE("QVR VSync timestamp is %0.2f in the future!", (float)(qvrTimeStamp - timestamp)* 0.000001f);
                qvrTimeStamp = timestamp;
            }

            if (gLogVSyncData)
            {
                LOGI("QVR last VSync Time: %0.4f ms ago!", (float)(timestamp - qvrTimeStamp) * 0.000001f);
            }
            ...
            if (((int64_t)(timestamp - qvrTimeStamp)) > framePeriodNano)
            {
                ...
                if (gLogVSyncData)
                {
                    LOGI("    Adding frame period time (%0.2f) to vsync time", framePeriodNano * 0.000001f);
                    LOGI("    Last VSync was %0.4f ms ago!", (float)(timestamp - gAppContext->modeContext->vsyncTimeNano) * 0.000001f);
                }
            }
            else
            {
        ...
        framePct = (double)gAppContext->modeContext->vsyncCount + ((double)timestamp - gAppContext->modeContext->vsyncTimeNano) / (framePeriodNano);
        ...
        if (fractFrame < gTimeWarpWaitFramePercentage)
        {
            ...
            waitTime = (uint64_t)((gTimeWarpWaitFramePercentage - fractFrame) * framePeriodNano);
        }
        else
        {
            //We are currently past the halfway point in the display so wait until halfway through the next vsync cycle
            waitTime = (uint64_t)((1.0 + gTimeWarpWaitFramePercentage - fractFrame) * framePeriodNano);
        }
        
        PROFILE_ENTER(GROUP_TIMEWARP, 0, "Left EyeBuffer Wait : %f", (double)(waitTime + warpTimeBiasNano) * 1e-6);
        ...
        LOGV("Warping %d (Ring = %d) [vc : %llu]", gAppContext->modeContext->warpFrameCount, gAppContext->modeContext->warpFrameCount % NUM_SWAP_FRAMES, gAppContext->modeContext->vsyncCount);
        ...
        {
            LOGI("Frame doubled on %d [%llu,%llu]", gAppContext->modeContext->warpFrameCount, (long long unsigned int)warpVsyncCount, (long long unsigned int)gAppContext->modeContext->vsyncCount);
        }
        ...
        if (gLogDroppedFrames && gFrameDoubled == 0 && gLastFrameIndx != 0)
        {
            ...
            {
                LOGE("Dropped Frame: Expected to warp frame %d, but got frame %d", gLastFrameIndx + 1, pWarpFrame->frameParams.frameIndex);
            }
        }
        ...
        PROFILE_ENTER(GROUP_TIMEWARP, 0, "Warp Submission : VSync %d Frame %d Doubled %d",
            ...
            (int)(pWarpFrame->frameParams.frameIndex),
            gFrameDoubled
        );
        ...
        // Since we support color planes, need RGB here
        glm::mat4 leftTextureMatrix[3];
        glm::mat4 rightTextureMatrix[3];
        glm::mat2 skewSquashMatrix = glm::mat2(1.0f);
        for (uint32_t whichPlane = 0; whichPlane < 3; whichPlane++)
        {
            leftTextureMatrix[whichPlane] = glm::mat4(1.0f);
            rightTextureMatrix[whichPlane] = glm::mat4(1.0f);
        }
        ...
        if (!gDisablePredictedTime && !(pWarpFrame->frameParams.frameOptions & kEnableMotionToPhoton))
        {
            // Predict ahead
        ...
        if (gLogPrediction)
        {
            ...
            if (leftWarpTime > expectedTime)
            {
                ...
                float diffTimeMs = (float)diffTimeNano * NANOSECONDS_TO_MILLISECONDS;
                LOGI("    (%0.3f ms) Left eye (Fame %d) predicted display time is %0.4f ms before warp expected display time!", timeSinceVrStart, pWarpFrame->frameParams.frameIndex, diffTimeMs);
            }
            else
            {
                ...
                float diffTimeMs = (float)diffTimeNano * NANOSECONDS_TO_MILLISECONDS;
                LOGI("    (%0.3f ms) Left eye (Fame %d) predicted display time is %0.4f ms after warp expected display time!", timeSinceVrStart, pWarpFrame->frameParams.frameIndex, diffTimeMs);
            }
        }
        ...
        unsigned int mesh1A_Camera = 0;    // First part of "Left" eye
        unsigned int mesh1B_Camera = 0;    // Second part of "Left" eye
        unsigned int mesh2A_Camera = 0;    // First part of "Right" eye
        unsigned int mesh2B_Camera = 0;    // Second part of "Right" eye
        ...
        case kMeshOrderLeftToRight:
            ...
            mesh1A_Camera = kMeshLeft_Camera;
            mesh1B_Camera = 0;
            mesh2A_Camera = kMeshRight_Camera;
            mesh2B_Camera = 0;
            break;
        ...
        case kMeshOrderRightToLeft:
            ...
            mesh1A_Camera = kMeshRight_Camera;
            mesh1B_Camera = 0;
            mesh2A_Camera = kMeshLeft_Camera;
            mesh2B_Camera = 0;
            break;

        case kMeshOrderTopToBottom:
            mesh1A = bDistortionEnabled ? kMeshLeft : kMeshLeft_Low;
            mesh1B = bDistortionEnabled ? kMeshRight : kMeshRight_Low;
            mesh2A = bDistortionEnabled ? kMeshLeft : kMeshLeft_Low;
            mesh2B = bDistortionEnabled ? kMeshRight : kMeshRight_Low;

            mesh1A_Sphere = kMeshLeft_Sphere;
            mesh1B_Sphere = kMeshRight_Sphere;
            mesh2A_Sphere = kMeshLeft_Sphere;
            mesh2B_Sphere = kMeshRight_Sphere;

            mesh1A_Camera = kMeshLeft_Camera;
            mesh1B_Camera = kMeshRight_Camera;
            mesh2A_Camera = kMeshLeft_Camera;
            mesh2B_Camera = kMeshRight_Camera;
            ...
            //mesh1A_Camera = kMeshUL_Camera;
            //mesh1B_Camera = kMeshUR_Camera;
            //mesh2A_Camera = kMeshLL_Camera;
            //mesh2B_Camera = kMeshLR_Camera;
            break;
        ...
        case kMeshOrderBottomToTop:
            ...
            //mesh1A_Camera = kMeshLL_Camera;
            //mesh1B_Camera = kMeshLR_Camera;
            //mesh2A_Camera = kMeshUL_Camera;
            //mesh2B_Camera = kMeshUR_Camera;
            break;
        }
        ...
        if (gEnableCameraLayer == true)
        {
            L_CameraUpdate(pWarpFrame);
        }
#endif

        glm::mat4 cameraWarpMatrix[3]; // Support color planes, need RGB here
        ...
        for (uint32_t whichPlane = 0; whichPlane < 3; whichPlane++)
        {
            leftTextureMatrix[whichPlane] =  eyeProjectionMatrix;
            rightTextureMatrix[whichPlane] =  eyeProjectionMatrix;
        }
        ...
        if (bTimeWarpEnabled)
        {
            ...
            glm::fquat headRotation = glmQuatFromSvrQuat(pWarpFrame->frameParams.headPoseState.pose.rotation);

            GetWarpMatrix(headRotation,
                          eyeProjectionMatrix, 
                          adjustedTimeMs, 
                          leftTextureMatrix); 
            ...
            for (uint32_t whichPlane = 0; whichPlane < 3; whichPlane++)
            { 
                rightTextureMatrix[whichPlane] = leftTextureMatrix[whichPlane];
            }

#if ENABLE_CAMERA
            /*
             * Calculate camera warp matrix for head rotation. Camera frames have their own
             * creation timestamp, which may be different from the other layers. 
             */
            if (gEnableCameraLayer == true)
            {
                for (uint32_t whichPlane = 0; whichPlane < 3; whichPlane++)
                {
                    cameraWarpMatrix[whichPlane] = glm::mat4(1.0f);
                }
                sxrHeadPoseState cameraPoseState = gSvrCameraManager.GetPoseState();
                glm::fquat cameraRotation = glmQuatFromSvrQuat(cameraPoseState.pose.rotation);
                //glm::fquat cameraRotation = glm::quat(1.0f, 0.f,0.f,0.f);
                GetWarpMatrix(cameraRotation,
                             eyeProjectionMatrix, 
                             adjustedTimeMs, 
                             cameraWarpMatrix); 
            }
            ...
            if (gApplyDisplaySquash || gApplyDisplaySkew)
            {
                float frameBias = gApplyDisplayBias ? gBiasScaleFactor : 1.0f;

                sxrHeadPoseState vsyncStartPoseState = sxrGetPredictedHeadPose(timeToVsyncMs);
                sxrHeadPoseState vsyncEndPoseState = sxrGetPredictedHeadPose(timeToVsyncMs + frameBias * (1000.0f / gAppContext->deviceInfo.displayRefreshRateHz));

                glm::fquat vsyncStartQuat = glmQuatFromSvrQuat(vsyncStartPoseState.pose.rotation);
                glm::fquat vsyncEndQuat = glmQuatFromSvrQuat(vsyncEndPoseState.pose.rotation);
                ...
                if (gApplyDisplaySquash)
                {
                    ...
                    {
                        float horizontalScale = -((gLeftFrustum_Near * 1000.0f) * tan(diffYaw)) / fovYmeters;
                    ...
                    {
                        float verticalScale = -((gLeftFrustum_Near * 1000.0f) * tan(diffPitch)) / fovYmeters;
        ...
        {
            sxrHeadPoseState displayPoseState = sxrGetPredictedHeadPose(0.0f);
            glm::fquat displayQuat = glmQuatFromSvrQuat(displayPoseState.pose.rotation);
            ...
            !gDirectModeWarp )
        {
            ...
            case kMeshOrderRightToLeft:
                gDirectModeWarp ? L_SetSurfaceScissor(kRightHalf) : L_StartTiledArea(kRightHalf);
                break;
            ...
            case kMeshOrderBottomToTop:
                gDirectModeWarp ? L_SetSurfaceScissor(kBottomHalf) : L_StartTiledArea(kBottomHalf);
                break;
            }
        ...
        if (!(pWarpFrame->frameParams.frameOptions & kEnableMotionToPhoton))
        {
            ...
            {
            case kMeshOrderLeftToRight:
                L_RenderOneEye(pWarpFrame, kEyeMaskLeft, kLeftHalf, mesh1A, mesh1A_Sphere, mesh1A_Camera, leftTextureMatrix, cameraWarpMatrix, skewSquashMatrix);
                break;

            case kMeshOrderRightToLeft:
                L_RenderOneEye(pWarpFrame, kEyeMaskRight, kRightHalf, mesh1A, mesh1A_Sphere, mesh1A_Camera, rightTextureMatrix, cameraWarpMatrix, skewSquashMatrix);
                break;

            case kMeshOrderTopToBottom:
                L_RenderOneEye(pWarpFrame, kEyeMaskLeft, kUpperLeft, mesh1A, mesh1A_Sphere, mesh1A_Camera, leftTextureMatrix, cameraWarpMatrix, skewSquashMatrix);
                L_RenderOneEye(pWarpFrame, kEyeMaskRight, kUpperRight, mesh1B, mesh1B_Sphere, mesh1B_Camera, rightTextureMatrix, cameraWarpMatrix, skewSquashMatrix);
                break;

            case kMeshOrderBottomToTop:
                L_RenderOneEye(pWarpFrame, kEyeMaskLeft, kLowerLeft, mesh1A, mesh1A_Sphere, mesh1A_Camera, leftTextureMatrix, cameraWarpMatrix, skewSquashMatrix);
                L_RenderOneEye(pWarpFrame, kEyeMaskRight, kLowerRight, mesh1B, mesh1B_Sphere, mesh1B_Camera, rightTextureMatrix, cameraWarpMatrix, skewSquashMatrix);
                break;
            }   // switch (gMeshOrderEnum)
        ...
        if (gSingleBufferWindow && gTimeWarpDelayBetweenEyes)
        {
            ...
            if (delta < ((uint64_t)(framePeriodNano / 2.0)) )
            {
                ...
                PROFILE_ENTER(GROUP_TIMEWARP, 0, "Right EyeBuffer Wait : %f", (double)(waitTime) * 1e-6);
                NanoSleep(waitTime);
        ...
        {
            gWarpFrameCount = -100;
            LOGI("Frame not displayed since not in VR mode");
        ...
        adjustedTimeNano = (uint64_t)(adjustedTimeMs * NANOSECONDS_TO_MILLISECONDS);
        ...
        if (gLogPrediction)
        {
            ...
            if (rightWarpTime > expectedTime)
            {
                ...
                float diffTimeMs = (float)diffTimeNano * NANOSECONDS_TO_MILLISECONDS;
                LOGI("    (%0.3f ms) Right eye (Fame %d) predicted display time is %0.4f ms before warp expected display time!", timeSinceVrStart, pWarpFrame->frameParams.frameIndex, diffTimeMs);
            }
            else
            {
                // The expected time was too far out
                ...
                float diffTimeMs = (float)diffTimeNano * NANOSECONDS_TO_MILLISECONDS;
                LOGI("    (%0.3f ms) Right eye (Fame %d) predicted display time is %0.4f ms after warp expected display time!", timeSinceVrStart, pWarpFrame->frameParams.frameIndex, diffTimeMs);
            }
        }
        ...
        if (bTimeWarpEnabled)
        {
            ...
            {
                /*
                 * Do nothing, use leftWarpTextureMatrix and rightWarpTextureMatrix from the 
                 * previous left eye calculation. 
                 */
            }
            else
            {
                /*
                 * Calculate normal layer texture matrix.
                 */
                glm::fquat headRotation = glmQuatFromSvrQuat(pWarpFrame->frameParams.headPoseState.pose.rotation);
                GetWarpMatrix(headRotation,
                              eyeProjectionMatrix, 
                ...
                for (uint32_t whichPlane = 0; whichPlane < 3; whichPlane++)
                {
                    leftTextureMatrix[whichPlane] = rightTextureMatrix[whichPlane];
                }
        ...
        if (!(pWarpFrame->frameParams.frameOptions & kEnableMotionToPhoton))
        {
            ...
            {
            case kMeshOrderLeftToRight:
                L_RenderOneEye(pWarpFrame, kEyeMaskRight, kRightHalf, mesh2A, mesh2A_Sphere, mesh2A_Camera, rightTextureMatrix, cameraWarpMatrix, skewSquashMatrix);
                break;

            case kMeshOrderRightToLeft:
                L_RenderOneEye(pWarpFrame, kEyeMaskLeft, kLeftHalf, mesh2A, mesh2A_Sphere, mesh2A_Camera, leftTextureMatrix,cameraWarpMatrix,  skewSquashMatrix);
                break;

            case kMeshOrderTopToBottom:
                L_RenderOneEye(pWarpFrame, kEyeMaskLeft, kLowerLeft, mesh2A, mesh2A_Sphere, mesh2A_Camera, leftTextureMatrix,cameraWarpMatrix,  skewSquashMatrix);
                L_RenderOneEye(pWarpFrame, kEyeMaskRight, kLowerRight, mesh2B, mesh2B_Sphere, mesh2B_Camera, rightTextureMatrix,cameraWarpMatrix,  skewSquashMatrix);
                break;

            case kMeshOrderBottomToTop:
                L_RenderOneEye(pWarpFrame, kEyeMaskLeft, kUpperLeft, mesh2A, mesh2A_Sphere, mesh2A_Camera, leftTextureMatrix,cameraWarpMatrix, skewSquashMatrix);
                L_RenderOneEye(pWarpFrame, kEyeMaskRight, kUpperRight, mesh2B, mesh2B_Sphere, mesh2B_Camera, rightTextureMatrix, cameraWarpMatrix, skewSquashMatrix);
                break;
            }   // switch (gMeshOrderEnum)
        ...
        if(gEnableRVR) {
            // Submit
            sxrFrameParams eyeFrameParams;
            memset(&eyeFrameParams, 0, sizeof(sxrFrameParams));
            eyeFrameParams.headPoseState = pWarpFrame->frameParams.headPoseState;
            eyeFrameParams.renderLayers[0].eyeMask = kEyeMaskLeft;
            eyeFrameParams.renderLayers[0].imageHandle = gWarpData.overlayTarget[0].GetColorAttachment();//gLeftEyeBuffer.GetColorAttachment();
            eyeFrameParams.renderLayers[0].layerFlags = 0;
            eyeFrameParams.renderLayers[1].eyeMask = kEyeMaskRight;
            eyeFrameParams.renderLayers[1].imageHandle = gWarpData.overlayTarget[1].GetColorAttachment();//gRightEyeBuffer.GetColorAttachment();
            eyeFrameParams.renderLayers[1].layerFlags = 0;
            if (gLogEyeRender) LOGI("[RVR] Before Submit (%d, %d) ts=%" PRIu64 " ", 
                gWarpData.overlayTarget[0].GetFrameBufferId(), gWarpData.overlayTarget[1].GetFrameBufferId(),
                eyeFrameParams.headPoseState.poseTimeStampNs);
            gRVRManager->SubmitFrame(&eyeFrameParams, &(gAppContext->modeContext->recenterRot), &(gAppContext->modeContext->recenterPos));
        }
#endif

#if ENABLE_CAMERA
    uint64_t now = now_ustw();
    if (gEnableCameraLayer == true)
    {
        gSvrCameraManager.ReleaseFrame(now, trendertop);
    }
#endif
...
#ifdef ENABLE_XR_CASTING
        PROFILE_ENTER(GROUP_TIMEWARP, 0, "SxrPresentation");
        // We only update the presentation when warp is done with a frame
        if (gEnableVRCasting
        && (gAppContext->isSxrPresentationInitialized)
        && (pWarpFrame != nullptr)
        && (gAppContext->modeContext->SxrPresentationLastFrameParams != nullptr)
        // This is checking that warp has moved onto a new frame so we can safely ues the lastFrame for cast
        && (gAppContext->modeContext->SxrPresentationLastFrameParams != pWarpFrame))
        {
            LOGV("SxrPresentation: Timewarp pthread_mutex_lock frame: %d", gAppContext->modeContext->warpFrameCount);
            pthread_mutex_lock(&gAppContext->modeContext->SxrPresentationFrameMutex);
            LOGV("SxrPresentation: Timewarp updated SxrPresentationPendingFrameTimeStamp: %" PRIu64 "", gAppContext->modeContext->SxrPresentationLastFrameParams->frameSubmitTimeStamp);
            gAppContext->modeContext->SxrPresentationPendingFrameTimeStamp = gAppContext->modeContext->SxrPresentationLastFrameParams->frameSubmitTimeStamp;
            gAppContext->modeContext->SxrPresentationPendingFrameParams = gAppContext->modeContext->SxrPresentationLastFrameParams;
            LOGV("SxrPresentation: Timewarp pthread_cond_signal frame: %d", gAppContext->modeContext->warpFrameCount);
            pthread_cond_signal(&gAppContext->modeContext->SxrPresentationFrameAvailableCV);
            LOGV("SxrPresentation: Timewarp pthread_mutex_unlock frame: %d", gAppContext->modeContext->warpFrameCount);
            pthread_mutex_unlock(&gAppContext->modeContext->SxrPresentationFrameMutex);
            LOGV("SxrPresentation: Timewarp submitted frame: %d", gAppContext->modeContext->warpFrameCount);
        }

        gAppContext->modeContext->SxrPresentationLastFrameParams = pWarpFrame;
        PROFILE_EXIT(GROUP_TIMEWARP); // SxrPresentation
#endif
...
//-----------------------------------------------------------------------------
{
    ...
    if (gAppContext->modeContext->eyeRenderContext == 0 && gAppContext->modeContext->eyeRenderWarpSurface == 0)
    {
    ...
    else
    {
        ...
        {
            LOGI("JCook: Even though svrUpdateEyeContextSurface failed, NOT leaving!");
            // return false;
        }
    }
    ...
    pthread_setname_np(gAppContext->modeContext->warpThread, "sxrWarp");
...
//-----------------------------------------------------------------------------
{
    ...
    if (pVertexData == NULL || pIndices == NULL)
    {
        ...
        gWarpData.warpMeshes[targetMesh].Destroy();
        return SXR_ERROR_NONE;
    }
    ...
    if (gWarpData.warpMeshes[targetMesh].GetVaoId() != 0)
    {
        // Already created, update the data
        if (gWarpData.warpMeshes[targetMesh].GetVertexCount() != nVertices ||
            gWarpData.warpMeshes[targetMesh].GetIndexCount() != nIndices)
        {
            LOGE("Unable to update warp mesh!  New mesh is not the same size as previous mesh!");
        }
        else
        {
            gWarpData.warpMeshes[targetMesh].Update(pVertexData, vertexSize, nVertices);
    ...
    else
    {
        ...
        gWarpData.warpMeshes[targetMesh].Initialize(&gMeshAttribs[0], NUM_VERT_ATTRIBS,
                                                    pIndices, nIndices,
...
{
    /*
     * Calculate warp for each RGB plane
     */
    for (uint32_t whichPlane = 0; whichPlane < 3; whichPlane++)
    {
        if (!gSupportColorPlaneWarp && whichPlane != 1)
        {
            // Since not doing color planes we only do "green"
            continue;
        }

        float colorAdjustMs = 0.0f;
        if (gSupportColorPlaneWarp)
        {
            switch (whichPlane)
            {
            case 0:
                colorAdjustMs = gColorPlaneTimeOffsetR;
                break;
            case 1:
                colorAdjustMs = gColorPlaneTimeOffsetG;
                break;
            case 2:
                colorAdjustMs = gColorPlaneTimeOffsetB;
                break;
            }
        }

        sxrHeadPoseState displayPoseState = sxrGetPredictedHeadPose(adjustedTimeMs + colorAdjustMs);
        glm::fquat displayQuat = glmQuatFromSvrQuat(displayPoseState.pose.rotation);

        glm::mat4 differenceMatrix = CalculateWarpMatrix(origQuat, displayQuat);
        pWarpMatrix[whichPlane] = differenceMatrix * eyeProjectionMatrix;
    }
}

#ifdef ENABLE_CAMERA
/**********************************************************
 * InitializeCameraWarpMesh
 **********************************************************/
void InitializeCameraWarpMesh(SvrAsyncWarpResources* pWarpData)
{
    if (gEnableCameraLayer == false)
    {
        // no need to make camera layer mesh, will not be used.
        return;
    }

    SxrResult result = gSvrCameraManager.Init();
    if (result != SXR_ERROR_NONE)
    {
        return;
    }

    if (gSvrCameraManager.IsSinglePass() == false)
    {
        // no need to make camera layer mesh, will not be used.
        return;
    }
    gSvrCameraManager.MakeOnePassMesh();
    return;
}
#endif
