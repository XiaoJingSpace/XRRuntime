...
void getQuaternion(JNIEnv * env, jobject quatObj, sxrFrameParams& mParams,
...
void getVector3(JNIEnv * env, jobject svrVector3Obj, sxrFrameParams& mParams,
...
void getHeadPoseState(JNIEnv * env, jclass rootClass, jobject rootObject, sxrFrameParams& mParams,
...
void getImageCoords(JNIEnv * env, jobject imageCoordsObj, sxrLayoutCoords& imageCoords, jfieldID* fid)
{
...
void getVulkanTexInfo(JNIEnv * env, jclass vulkanInfoClass, jobject vulkanInfoObj, sxrVulkanTexInfo& vulkanInfo)
{
    // uint32_t            memSize;
...
void setVec3Object(JNIEnv * env, jobject& targetObj, jfieldID* hpxFid, sxrVector3 vec)
{
...
void setViewFrustum(JNIEnv * env, jobject& targetObj, jfieldID* vfFid, sxrViewFrustum vfum)
{
    // float               left; 
...
{
    jclass beginParamsLocal = env->FindClass("com/qualcomm/sxrapi/SxrApi$sxrBeginParams");
    *(apiClazz + JCLASS_BPARAM) = (jclass)env->NewGlobalRef(beginParamsLocal);

    jclass frameParamsLocal = env->FindClass("com/qualcomm/sxrapi/SxrApi$sxrFrameParams");
    *(apiClazz + JCLASS_FPARAM) = (jclass)env->NewGlobalRef(frameParamsLocal);

    *(apiClazz + JCLASS_PERFLEV) = env->FindClass("com/qualcomm/sxrapi/SxrApi$sxrPerfLevel");

    *(apiClazz + JCLASS_EYELAYER) = env->FindClass("com/qualcomm/sxrapi/SxrApi$sxrRenderLayer");

    jclass layoutCoordsLocal = env->FindClass("com/qualcomm/sxrapi/SxrApi$sxrLayoutCoords");
    *(apiClazz + JCLASS_LAYOUTCOORDS) = (jclass)env->NewGlobalRef(layoutCoordsLocal);

    jclass vulkanInfoLocal = env->FindClass("com/qualcomm/sxrapi/SxrApi$sxrVulkanTexInfo");
    *(apiClazz + JCLASS_VULKANINFO) = (jclass)env->NewGlobalRef(vulkanInfoLocal);

    *(apiClazz + JCLASS_TEXTURETYPE) = env->FindClass("com/qualcomm/sxrapi/SxrApi$sxrTextureType");

    *(apiClazz + JCLASS_EYEMASK) = env->FindClass("com/qualcomm/sxrapi/SxrApi$sxrEyeMask");

    *(apiClazz + JCLASS_WARPTYPE) = env->FindClass("com/qualcomm/sxrapi/SxrApi$sxrWarpType");

    jclass hpStateTmp = env->FindClass("com/qualcomm/sxrapi/SxrApi$sxrHeadPoseState");
    *(apiClazz + JCLASS_HEADPOSESTATE) = (jclass)env->NewGlobalRef(hpStateTmp);

    jclass hposeTmp = env->FindClass("com/qualcomm/sxrapi/SxrApi$sxrHeadPose");
    *(apiClazz + JCLASS_HEADPOSE) = (jclass)env->NewGlobalRef(hposeTmp);

    jclass quatTemp = env->FindClass("com/qualcomm/sxrapi/SxrApi$sxrQuaternion");
    *(apiClazz + JCLASS_QUAT) = (jclass)env->NewGlobalRef(quatTemp);

    jclass vec3Temp = env->FindClass("com/qualcomm/sxrapi/SxrApi$sxrVector3");
    *(apiClazz + JCLASS_VEC3) = (jclass)env->NewGlobalRef(vec3Temp);

    *(apiClazz + JCLASS_WHICHEYE) = env->FindClass("com/qualcomm/sxrapi/SxrApi$sxrWhichEye");

    jclass dvClassTmp = env->FindClass("com/qualcomm/sxrapi/SxrApi$sxrDeviceInfo");
    *(apiClazz + JCLASS_DVINFO) = (jclass)env->NewGlobalRef(dvClassTmp);

    jclass viemFruTmp = env->FindClass("com/qualcomm/sxrapi/SxrApi$sxrViewFrustum");
    *(apiClazz + JCLASS_VFRU) = (jclass)env->NewGlobalRef(viemFruTmp);

    *(apiClazz + JCLASS_WARPMESHENUM) = env->FindClass("com/qualcomm/sxrapi/SxrApi$sxrWarpMeshEnum");
    *(apiClazz + JCLASS_COLORSPACE) = env->FindClass("com/qualcomm/sxrapi/SxrApi$sxrColorSpace");
}
...
void initDeviceInfoFid(JNIEnv * env, jfieldID* dvFid, jclass dvInfo, jclass vfrum)
{
    ...
    *(dvFid + DV_VFRUM_LEFT) = env->GetFieldID(dvInfo,
        "leftEyeFrustum", "Lcom/qualcomm/sxrapi/SxrApi$sxrViewFrustum;");
    //  public sxrViewFrustum  rightEyeFrustum; 
    *(dvFid + DV_VFRUM_RIGHT) = env->GetFieldID(dvInfo,
        "rightEyeFrustum", "Lcom/qualcomm/sxrapi/SxrApi$sxrViewFrustum;");
    ...
    jclass beginParam, jclass frameParam, jclass renderLayer)
{
    ...
    *(apiFiledID + FID_FPARAM_EYELAYER) = env->GetFieldID(frameParam,
        "renderLayers", "[Lcom/qualcomm/sxrapi/SxrApi$sxrRenderLayer;");
    ...
    *(apiFiledID + FID_FPARAM_WARPTYPE) = env->GetFieldID(frameParam,
        "warpType", "Lcom/qualcomm/sxrapi/SxrApi$sxrWarpType;");
    ...
    jclass hpState, jclass headPose, jclass quat, jclass vec3)
{
    ...
    *(hpXFid + HP_FPARAM_HPSTATE) = env->GetFieldID(frameParam,
        "headPoseState", "Lcom/qualcomm/sxrapi/SxrApi$sxrHeadPoseState;");
    //sxrHeadPose Obj
    *(hpXFid + HP_POSE) = env->GetFieldID(hpState,
        "pose", "Lcom/qualcomm/sxrapi/SxrApi$sxrHeadPose;");
    //get rotation fid in sxrHeadPose
    *(hpXFid + HP_ROTATION) = env->GetFieldID(headPose,
        "rotation", "Lcom/qualcomm/sxrapi/SxrApi$sxrQuaternion;");
    //rotation x
    ...
    *(hpXFid + HP_POSITION) = env->GetFieldID(headPose,
        "position", "Lcom/qualcomm/sxrapi/SxrApi$sxrVector3;");
    //rotation x
...
void initMethodID(JNIEnv * env, jclass clazz, jmethodID* apiMethodID, jclass* apiClazz)
{
    ...
    *(apiMethodID + MID_HPSTATE_INIT) = env->GetMethodID(*(apiClazz + JCLASS_HEADPOSESTATE),
        "<init>", "(Lcom/qualcomm/sxrapi/SxrApi;)V");
...
{
    //-----------------------------------------------------------------------------
    void Java_com_qualcomm_sxrapi_SxrApi_sxrInitialize(JNIEnv * env, jclass clazz, jobject thiz)
    //-----------------------------------------------------------------------------
    {
    ...
    //-----------------------------------------------------------------------------
    void Java_com_qualcomm_sxrapi_SxrApi_sxrBeginXr(JNIEnv * env, jclass clazz, jobject thiz, jobject mBeginParamsObj)
        //-----------------------------------------------------------------------------
    {
        ...
        LOG_EGL_RESOURCE("%s:%i:threadid=%i, beginParams.nativeWindow=ANativeWindow_fromSurface(surface=%x)=%p", __FILE__, __LINE__, (size_t)surface, beginParams.nativeWindow);

        //get cpu/gpu perf level
        jobject cpuPerfLevelObj = env->GetObjectField(mBeginParamsObj, apiFiledID[FID_BPARAM_CPUPERF]);
        beginParams.cpuPerfLevel = (sxrPerfLevel)env->CallIntMethod(cpuPerfLevelObj, apiMethodID[MID_PERFLEV_ORDINAL]);

        jobject gpuPerfLevelObj = env->GetObjectField(mBeginParamsObj, apiFiledID[FID_BPARAM_GPUPERF]);
        beginParams.gpuPerfLevel = (sxrPerfLevel)env->CallIntMethod(gpuPerfLevelObj, apiMethodID[MID_PERFLEV_ORDINAL]);
        ...
        beginParams.colorSpace = (sxrColorSpace)env->CallIntMethod(colorSpaceObj, apiMethodID[MID_COLORSPACE_ORDINAL]);

        beginParams.mainThreadId = gettid();

        if (sxrBeginXr(&beginParams) != SXR_ERROR_NONE)
        {
            LOGE("sxrBeginXR failed!");
    ...
    //-----------------------------------------------------------------------------
    void Java_com_qualcomm_sxrapi_SxrApi_sxrSubmitFrame(JNIEnv * env, jclass clazz, jobject thiz, jobject frameParamsObj)
        //-----------------------------------------------------------------------------
    {
        //get frameIndex
        ...
        jobject eyeLayersObj[SXR_MAX_RENDER_LAYERS];
        ...
        for (int i = 0; i < SXR_MAX_RENDER_LAYERS; i++)
        {
            ...
            frameParams.renderLayers[i].imageType =
                (sxrTextureType)env->CallIntMethod(imageTypeObj, apiMethodID[MID_TEXTYPE_ORDINAL]);
            //imageCoords
            ...
            //public sxrEyeMask          eyeMask;            //!< Determines which eye[s] receive this render layer
            eyeMaskObj = env->GetObjectField(eyeLayersObj[i], apiFiledID[FID_RENDERLAYER_EYEMASK]);
            frameParams.renderLayers[i].eyeMask =
                (sxrEyeMask)env->CallIntMethod(eyeMaskObj, apiMethodID[MID_EYEMASK_GETVALUE]);
            //layerFlags
        ...
        frameParams.warpType =
            (sxrWarpType)env->CallIntMethod(warpTypeObj, apiMethodID[MID_WARPTYPE_ORDINAL]);
        //public float                fieldOfView;
    ...
    void Java_com_qualcomm_sxrapi_SxrApi_sxrEndXr(JNIEnv * env, jclass clazz)

    {
        if (sxrEndXr() != SXR_ERROR_NONE)
        {
            LOGE("sxrEndXR failed!");
        }
    }

    void Java_com_qualcomm_sxrapi_SxrApi_sxrShutdown(JNIEnv * env, jclass clazz)
    {
        if (sxrShutdown() != SXR_ERROR_NONE)
        {
            LOGE("sxrEndXR failed!");
        }
        deleteGlobalRef(env, apiClazz);
    ...
    JNIEXPORT jstring JNICALL
        Java_com_qualcomm_sxrapi_SxrApi_sxrGetVersion(JNIEnv * env, jclass clazz)

    {
        char buf[256];
        memset(buf, 0, sizeof(buf));
        strcpy(buf, sxrGetVersion());
        jstring jstrBuf = env->NewStringUTF(buf);
        return jstrBuf;
    }

    JNIEXPORT jstring JNICALL
        Java_com_qualcomm_sxrapi_SxrApi_sxrGetXrServiceVersion(JNIEnv * env, jclass clazz)

    {
        char buf[256];
        memset(buf, 0, sizeof(buf));
        sxrGetXrServiceVersion(buf, 100);
        jstring jstrBuf = env->NewStringUTF(buf);
        return jstrBuf;
    }

    JNIEXPORT jstring JNICALL
        Java_com_qualcomm_sxrapi_SxrApi_sxrGetXrClientVersion(JNIEnv * env, jclass clazz)

    {
        char buf[256];
        memset(buf, 0, sizeof(buf));
        sxrGetXrClientVersion(buf, 100);
        jstring jstrBuf = env->NewStringUTF(buf);
        return jstrBuf;
    }

    JNIEXPORT jfloat JNICALL
        Java_com_qualcomm_sxrapi_SxrApi_sxrGetPredictedDisplayTime(JNIEnv * env, jclass clazz)

    {
        jfloat predictedTimeMs = sxrGetPredictedDisplayTime();
        LOGI("Predicted Display Time : %0.3f", predictedTimeMs);
        return predictedTimeMs;
    }

    JNIEXPORT jfloat JNICALL
        Java_com_qualcomm_sxrapi_SxrApi_sxrGetPredictedDisplayTimePipelined(JNIEnv * env, jclass clazz, jint depth)

    {
        jfloat predictedTimeMs = sxrGetPredictedDisplayTimePipelined(depth);
        LOGI("Predicted Display Time : %0.3f", predictedTimeMs);
        return predictedTimeMs;
    }
    ...
    * Signature: (F)Lcom/qualcomm/sxrapi/SxrApi/svrHeadPoseState;
    */
    JNIEXPORT jobject JNICALL
        Java_com_qualcomm_sxrapi_SxrApi_sxrGetPredictedHeadPose(JNIEnv * env, jclass clazz, jfloat predictedTimeMs)
    {
        sxrHeadPoseState poseState = sxrGetPredictedHeadPose(predictedTimeMs);
    ...
    * Signature: (Lcom/qualcomm/sxrapi/SxrApi/sxrPerfLevel;
    *Lcom/qualcomm/sxrapi/SxrApi/sxrPerfLevel;)Lcom/qualcomm/sxrapi/SxrApi/SvrResult;*/

    JNIEXPORT void JNICALL
        Java_com_qualcomm_sxrapi_SxrApi_sxrSetPerformanceLevels(JNIEnv * env, jclass clazz, jobject cpuPerfLevel, jobject gpuPerfLevel)
    {
        sxrPerfLevel cpuPerfLev = (sxrPerfLevel)env->CallIntMethod(cpuPerfLevel, apiMethodID[MID_PERFLEV_ORDINAL]);
        sxrPerfLevel gpuPerfLev = (sxrPerfLevel)env->CallIntMethod(gpuPerfLevel, apiMethodID[MID_PERFLEV_ORDINAL]);

        SxrResult setPerfResult = sxrSetPerformanceLevels(cpuPerfLev, gpuPerfLev);
    ...
    * Signature: ()Lcom/qualcomm/sxrapi/SxrApi/svrDeviceInfo;
    */
    JNIEXPORT jobject JNICALL
        Java_com_qualcomm_sxrapi_SxrApi_sxrGetDeviceInfo(JNIEnv * env, jclass clazz)
    {
    ...
    * Signature: ()Lcom/qualcomm/sxrapi/SxrApi/SvrResult;
    */
    JNIEXPORT void JNICALL
        Java_com_qualcomm_sxrapi_SxrApi_sxrRecenterPose(JNIEnv * env, jclass clazz)
    {
        SxrResult sxrResultValue = sxrRecenterPose();
        setSvrResultValue(env, clazz, sxrResultValue);
    }
    ...
    */
    JNIEXPORT void JNICALL
        Java_com_qualcomm_sxrapi_SxrApi_sxrRecenterPosition(JNIEnv * env, jclass clazz)
    {
        SxrResult sxrResultValue = sxrRecenterPosition();
        setSvrResultValue(env, clazz, sxrResultValue);
    }
    ...
    * Signature: (Z)Lcom/qualcomm/sxrapi/SxrApi/SvrResult;
    */
    JNIEXPORT void JNICALL
        Java_com_qualcomm_sxrapi_SxrApi_sxrRecenterOrientation(JNIEnv * env, jclass clazz, jboolean yawOnly)
    {
        SxrResult sxrResultValue = sxrRecenterOrientation(yawOnly);
        setSvrResultValue(env, clazz, sxrResultValue);
    }
    ...
    */
    JNIEXPORT jint JNICALL
        Java_com_qualcomm_sxrapi_SxrApi_sxrGetSupportedTrackingModes(JNIEnv * env, jclass clazz)
    {
        jint supportedTrackingModes = sxrGetSupportedTrackingModes();
    ...
    * Signature: (I)Lcom/qualcomm/sxrapi/SxrApi/SvrResult;
    */
    JNIEXPORT void JNICALL
        Java_com_qualcomm_sxrapi_SxrApi_sxrSetTrackingMode(JNIEnv * env, jclass clazz, jint trackingModes)
    {
        SxrResult sxrResultValue = sxrSetTrackingMode(trackingModes);
        setSvrResultValue(env, clazz, sxrResultValue);
    }
    ...
    */
    JNIEXPORT jint JNICALL
        Java_com_qualcomm_sxrapi_SxrApi_sxrGetTrackingMode(JNIEnv * env, jclass clazz)
    {
        jint trackingMode = sxrGetTrackingMode();
        return trackingMode;
    }
    ...
    * Class:     com_qualcomm_svrapi_SvrApi
    * Method:    svrBeginEye
    * Signature: (Lcom/qualcomm/sxrapi/SxrApi/svrEyeBufferType;
    *Lcom/qualcomm/sxrapi/SxrApi/svrWhichEye;)Lcom/qualcomm/sxrapi/SxrApi/SvrResult;
    */
    JNIEXPORT void JNICALL
        Java_com_qualcomm_sxrapi_SxrApi_sxrBeginEye(JNIEnv * env, jclass clazz, jobject whichEye)
    {
        sxrWhichEye sxrWhichEyeValue = (sxrWhichEye)env->CallIntMethod(whichEye, apiMethodID[MID_WHICHEYE_ORDINAL]);
        SxrResult sxrResultValue = sxrBeginEye(sxrWhichEyeValue);
        setSvrResultValue(env, clazz, sxrResultValue);
    }
    ...
    */
    JNIEXPORT void JNICALL
        Java_com_qualcomm_sxrapi_SxrApi_sxrEndEye(JNIEnv * env, jclass clazz, jobject whichEye)
    {
        sxrWhichEye sxrWhichEyeValue = (sxrWhichEye)env->CallIntMethod(whichEye, apiMethodID[MID_WHICHEYE_ORDINAL]);
        SxrResult sxrResultValue = sxrEndEye(sxrWhichEyeValue);
        setSvrResultValue(env, clazz, sxrResultValue);
    }
    ...
    */
    JNIEXPORT void JNICALL
        Java_com_qualcomm_sxrapi_SxrApi_svrSetWarpMesh(JNIEnv * env, jclass clazz, jobject whichMesh,
            jfloatArray pVertexData, jint vertexSize, jint nVertices, jintArray pIndices, jint nIndices)
    {
        sxrWarpMeshEnum warpMeshEnum =
            (sxrWarpMeshEnum)env->CallIntMethod(whichMesh, apiMethodID[MID_WARPMESH_ORDINAL]);
        ...
        SxrResult sxrResultValue = sxrSetWarpMesh(warpMeshEnum, pVertexDataInJava,
    ...
    */
    JNIEXPORT void JNICALL
        Java_com_qualcomm_sxrapi_SxrApi_sxrGetOcclusionMesh(JNIEnv * env, jclass clazz, jobject whichEye,
            jintArray pTriangleCount, jintArray pVertexStride, jfloatArray pTriangles)
    {
        ...
        SxrResult sxrResultValue = sxrGetOcclusionMesh(sxrWhichEyeValue, pTriangleCountInJava,
    ...
    JNIEXPORT void JNICALL
    Java_com_qualcomm_sxrapi_xrcasting_SxrPresentation_sxrSetPresentationSurfaceChanged(JNIEnv * env, jclass clazz,
            jobject surface, jint format, jint width, jint height)

    {
#if ENABLE_XR_CASTING	    
        if (surface != NULL)
        {
            ANativeWindow* window = ANativeWindow_fromSurface(env, surface);
            sxrSetPresentationSurfaceChanged(window, format, width, height);
        }
        else
        {
    ...
    JNIEXPORT void JNICALL
    Java_com_qualcomm_sxrapi_xrcasting_SxrPresentation_sxrSetPresentationSurfaceDestroyed(JNIEnv * env, jclass clazz,
                                                                                        jobject surface)
