...
void* SharedMemory::map(int fd, int fdSize, bool bReadOnly)
{
    ...
    LOGE("map fd(%d) fdSize(%d) getSize(%d)", fd, fdSize, getSize(fd));
...
{    
	...
	//The method of allocating sherad memory have been changed to NDK's ASharedMemory_create since Android api 26
	int fd = ASharedMemory_create("my_mem",size);
