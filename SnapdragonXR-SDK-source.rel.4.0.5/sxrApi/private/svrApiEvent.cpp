...
namespace Svr
{
    ...
    //-----------------------------------------------------------------------------
    bool CompareEvents(const sxrEvent& lhs, const sxrEvent& rhs)
    //-----------------------------------------------------------------------------
    {
    ...
    //-----------------------------------------------------------------------------
    {
        mpEventQueue = new spsc_bounded_queue_t<sxrEvent>(capacity);
        mThreadId = gettid();
        mBaseTime = baseTime;
    }
    ...
    //-----------------------------------------------------------------------------
    void svrEventQueue::SubmitEvent(sxrEventType eventType, sxrEventData& eventData, unsigned int deviceId)
    //-----------------------------------------------------------------------------
    {
        sxrEvent evt;
    ...
    //-----------------------------------------------------------------------------
    bool svrEventManager::PollEvent(sxrEvent& outEvent)
    //-----------------------------------------------------------------------------
    {
    ...
    //-----------------------------------------------------------------------------
    {
        static sxrEvent frameEvents[MAX_EVENTS_PER_QUEUE * MAX_STAGING_EVENT_QUEUES];

        memset(&frameEvents[0], 0, sizeof(sxrEvent) * MAX_EVENTS_PER_QUEUE * MAX_STAGING_EVENT_QUEUES);
