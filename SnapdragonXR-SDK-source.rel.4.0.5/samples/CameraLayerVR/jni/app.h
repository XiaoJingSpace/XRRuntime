//=============================================================================
// FILE: app.h
//
//                  Copyright (c) 2018 QUALCOMM Technologies Inc.
//                              All Rights Reserved.
//
//=============================================================================

#pragma once

#include <math.h>
#include <vector>
#include <string>
#include "sxrApiAnchors.h"
#include "sxrApi.h"
#include "svrApplication.h"
#include "svrGeometry.h"
#include "svrRenderTarget.h"
#include "svrShader.h"
#include "svrUtil.h"
#include "svrInput.h"
#include "glm/glm.hpp"
#include <glm/gtc/matrix_inverse.hpp>
#include <glm/gtc/matrix_transform.hpp>
#include <glm/gtc/quaternion.hpp>
#include <glm/gtc/type_ptr.hpp>
#include <glm/gtx/euler_angles.hpp>
#include <glm/gtx/normal.hpp>
#include <glm/gtx/quaternion.hpp>
#include <glm/gtx/transform.hpp>

#include "utils.h"

class CameraLayerApp : public Svr::SvrApplication {

public:
    CameraLayerApp();

    void Initialize();

    void LoadConfiguration() {};

    void Update();

    void Render();

    void Shutdown();

    void InitializeModel();

    void RenderEye(Svr::SvrEyeId);

    void SubmitFrame();

    float GetRotationAmount(unsigned int, float);

private:
    //This app will render six objects at each direction(up,down,left,right,forward,backward).
    enum
    {
        MODEL_SIZE = 6
    };

    //Use SvrGeometry to load model from an .obj file
    Svr::SvrGeometry *mModel;
    Svr::SvrPoints mPointCloud;
    //Svr::SvrGeometry mPointCloud;

    //Use SvrShader to load vertex and fragment shaders from file.
    Svr::SvrShader mShader;
    Svr::SvrShader mPointCloudShader;

    //Use a texture for coloring
    GLuint mModelTexture;
    //overlay texture 
    GLuint mOverlayTexture;

    //pose state
    sxrHeadPoseState mPoseState;

    //view matrix
    glm::mat4 mViewMatrix[kNumEyes];

    //projection matrix
    glm::mat4 mProjectionMatrix[kNumEyes];

    // model matrix for 3dr objects
    sxrMatrix mQvrDataTransformMat;
    glm::mat4 mQvrToSxrMat;
    glm::mat4 mSxrToQvrMat;

    //model matrix for each object
    glm::mat4 mModelMatrix[MODEL_SIZE];

    //color for each object
    glm::vec3 mModelColor[MODEL_SIZE];
    glm::vec4 mPointCloudColor;

    float mPointCloudSize;
    //std::vector<sxrCloudPoint> mCloudPoints;
    std::vector<uint32_t> mCloudIndices;

    float mAnchorSize;
    std::vector<glm::mat4> mAnchorMatrix;
    std::vector<sxrAnchorUuid> mAnchorId;

    //recored the render time to calculate rotation
    unsigned int mLastRenderTime;
    unsigned int mLastToggleTime;

    static constexpr float RADIAN_ONE = M_PI/180.0f;

};