...
//Note: If the eye buffers are subsampled textures, use GL_FOVEATION_SUBSAMPLED_LAYOUT_METHOD_BIT_QCOM
// for further reduced gpu update times.
//That's it!
...
{
    mMsaaSamples = 2; // Use msaa > 1 to enable subsampled buffers
    mLastRenderTime = 0;
    mModelTexture = 0;
    mFocalPointX = 0.0f;
    mFocalPointY = 0.0f;
...
{
    //mAppContext.isFoveationSubsampled = true;
...
void FoveatedVR::AllocateEyeBuffers()
{
    ...
    GLuint foveationFeatureBits = mAppContext.isFoveationSubsampled ?
    ...
    for (int i = 0; i < SVR_NUM_EYE_BUFFERS; i++) 
	{
        mAppContext.eyeBuffers[i].eyeTarget[kLeft].Initialize(mAppContext.targetEyeWidth,
                                                              mAppContext.targetEyeHeight, mMsaaSamples,
                                                              GL_RGBA8, true);
        ...
        if (IsFoveatedSupported()) 
		{
            ...
            GL(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_FOVEATED_FEATURE_BITS_QCOM, foveationFeatureBits));
...
void FoveatedVR::Update()
{
	...
	// custom per-eye translations & rotations would go here
	glm::vec3 zeroIPD(0.f,0.01f,0.f);  // a zero IPD means use defaault IPD, but we really want 0, so make it almost zero
	const glm::fquat identity(1.0f, 0.0f, 0.0f, 0.0f); // glm quat is (w)(xyz)
	
    SvrGetEyeViewMatrices(mPoseState, true,
                            zeroIPD, zeroIPD, identity, identity,
							DEFAULT_HEAD_HEIGHT, DEFAULT_HEAD_DEPTH,
                            mViewMatrix[kLeft], mViewMatrix[kRight]);
    ...
    if (mControllerHandler != CONTROLLER_UNINIT) {
        sxrControllerState controllerState = sxrControllerGetState(mControllerHandler);
...
void FoveatedVR::StartController()
{
    ...
    mControllerHandler = sxrControllerStartTracking("foveated-test");
    if (mControllerHandler == CONTROLLER_UNINIT) {
    ...
    sxrControllerState state = sxrControllerGetState(mControllerHandler);
