...
// Pull in the SXR version of log macros
#include "sxrApi.h"
#include "svrUtil.h"

// Helper define for tracking error locations in code
#define VK_CHECK(X) if (!(X)) { LOGE("VK_CHECK Failure -> (Line %d) %s", __LINE__, __FILE__); assert((X));}
...
public:
    ...
    // Sample overrides
    virtual void	PointerDownEvent(int pointerId, float x, float y) = 0;
    virtual void	PointerUpEvent(int pointerId, float x, float y) = 0;
    virtual void	PointerMoveEvent(int pointerId, float x, float y) = 0;
