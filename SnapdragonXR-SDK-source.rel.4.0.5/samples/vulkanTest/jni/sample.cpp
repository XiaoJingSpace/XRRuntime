...
#include "glm/glm.hpp"  // for SvrGetEyeViewMatrices arguments
...
sxrHeadPoseState gCurrentPoseState;
...
void StartVR(android_app *pApp)
{
    ...
    {
        LOGI("Calling sxrSetTrackingMode (0x%x)...", kTrackingRotation | kTrackingPosition);
...
void L_CreateLayout(float centerX, float centerY, float radiusX, float radiusY, sxrLayoutCoords *pLayout)
{
...
bool VkSample::Update()
{
    ...
    gCurrentPoseState = sxrGetPredictedHeadPose(predictedTimeMs);
	...
	// add in default IPD
	glm::vec3 leftIPD(-0.5*DEFAULT_IPD,0.f,0.f);
	glm::vec3 rightIPD(-leftIPD);

    SvrGetEyeViewMatrices(gCurrentPoseState,
		true, // use a head model?
		LeftEyePos+leftIPD, RightEyePos+rightIPD,
