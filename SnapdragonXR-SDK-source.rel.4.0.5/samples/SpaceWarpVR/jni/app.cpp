...
//
//=============================================================================
//    Async Space Warp (aka Motion Vectors) with Snapdragon VRSDK Native app
//=============================================================================
...
//=============================================================================
//In this app, we will force missed frames and use ASW to fill in those frames.
//=============================================================================
#include "app.h"
#include <unistd.h>
...
{
    gForcedVSyncTime = 30;
    mAppContext.isMotionAwareFrames = true;
    //mAppContext.isColorSpaceSRGB = true;
    SvrApplication::Initialize();
    InitializeModel();
}

//Allocate eye buffers, with MSAA
void SimpleApp::AllocateEyeBuffers()
{
    for (int whichBuffer = 0; whichBuffer < SVR_NUM_EYE_BUFFERS; whichBuffer++)
    {
        mAppContext.eyeBuffers[whichBuffer].eyeTarget[kLeft].Initialize(mAppContext.targetEyeWidth,
            mAppContext.targetEyeHeight, mMsaaSamples, GL_RGBA8, true, false);
...
void SimpleApp::Render()
{
    ...
    {
        // Forcing VSync
        int frameTime = TimeNow - gLastRenderTime;
        if (frameTime < gForcedVSyncTime)
        {
            //return;
            int sleepTime = (gForcedVSyncTime - frameTime) * 1000;
            usleep(sleepTime);
        }
    }
    gLastRenderTime = TimeNow;
