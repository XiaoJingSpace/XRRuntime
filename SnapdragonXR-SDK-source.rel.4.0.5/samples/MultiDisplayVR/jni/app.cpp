...
//Snapdragon VRSDK that displays on a secondary screen, while a regular app 
//displays on the phone.  The app on the phone can send arbitrary information 
//to the app on the secondary screen.
...
// don't know how to render something with OpenGLES on Android NativeActivity.
//
//Based on the Android NativeActivity, Snapdragon VRSDK  provide svr framework
// layer, which helps handle the Activity lifecycle and VR modes switch; this 
// layer provide a base class SvrApplication.  It is easy to inherit
// this class and implement the related callbacks(Initialize,Update,Render) to
// create your own VR apps.
//Please note that currently suspend/resume with the physical power button and 
//screenlock enabled is not supported; this appears to trigger an EGL/Android bug.
//
...
// need get the head pose each frame(refer sxrGetPredictedHeadPose and
...
static float s_colorScalar=1.f;
...
void SimpleApp::Update()
{
    ...
    mPoseState = sxrGetPredictedHeadPose(predDispTime);
...
void SimpleApp::RenderEye(Svr::SvrEyeId eyeId)
{
    ...
    for (int j = 0; j < MODEL_SIZE; j++) {
        ...
        glm::vec3 finalColor = mModelColor[j] * s_colorScalar;
        mShader.SetUniformVec3("modelColor", finalColor);
        mModel->Submit();
    }
    mShader.Unbind();
...
extern "C"
{
    JNIEXPORT void JNICALL Java_com_qualcomm_sxr_multidisplayvr_SxrNativeActivity_nativeColorScalarSet(JNIEnv *jni, jobject, jfloat colorScalar)
    {
        s_colorScalar = colorScalar;
    }
}