...
{
	LOGE("MPS %s(): samples = %d, mIsProtectedContent = %d\n ", __FUNCTION__ , samples, mIsProtectedContent);
...
void SvrRenderTarget::InitializeMultiSampleTexture2DMSEXT(int width, int height, int samples, int colorSizedFormat, int format, int type, bool requiresDepth)
{
	LOGE("MPS %s(): samples = %d, mIsProtectedContent = %d\n ", __FUNCTION__ , samples, mIsProtectedContent);
	...
	GL(glFramebufferTexture2DMultisampleEXT(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, mColorAttachmentId, 0, samples));
	if (requiresDepth)
	{
		GL(glFramebufferTexture2DMultisampleEXT(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, mDepthAttachmentId, 0, samples));
	}
...
{
	LOGE("MPS %s(): samples = %d, isProtectedContent = %d\n ", __FUNCTION__ , samples, isProtectedContent);
	...
	if (samples > 1)
	{
		...
		InitializeMultiSampleTexture2DMSEXT(width, height, samples, colorSizedFormat, format, type, requiresDepth);
	}
	else
	{
...
{
	LOGE("MPS %s(): mSamples = %d, mIsProtectedContent = %d\n ", __FUNCTION__ , mSamples, mIsProtectedContent);
