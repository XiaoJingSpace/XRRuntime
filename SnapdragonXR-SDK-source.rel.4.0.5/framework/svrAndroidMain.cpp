...
#define ANDROID_FRAMEWORK_STATE_LOGGING_ENABLED 0
#if ANDROID_FRAMEWORK_STATE_LOGGING_ENABLED
#define LOG_ANDROID_FRAMEWORK_STATE(...) __android_log_print( ANDROID_LOG_VERBOSE, "sxr", __VA_ARGS__ )
#else
#define LOG_ANDROID_FRAMEWORK_STATE(...)
#endif
...
void PauseVR(Svr::SvrApplication* pApp);
void StartVR();
void EndVR(SvrApplication*);
...
static void deleteContext(SvrApplicationContext& appContext)
{
    ...
    const EGLBoolean destroySurfaceResult = eglDestroySurface(appContext.display, appContext.eyeRenderSurface);
    LOG_EGL_RESOURCE("%s:%i:threadid=%i, eglDestroySurface(display=%p, eyeRenderSurface=%p)=%i", __FILE__, __LINE__, gettid(), appContext.display, appContext.eyeRenderSurface, destroySurfaceResult);

    const EGLBoolean destroyContextResult = eglDestroyContext(appContext.display, appContext.eyeRenderContext);
    LOG_EGL_RESOURCE("%s:%i:threadid=%i, eglDestroyContext(display=%p, eyeRenderContext=%p)=%i", __FILE__, __LINE__, gettid(), appContext.display, appContext.eyeRenderContext, destroyContextResult);

    //ensure any current surface or context is destroyed rather than kept because it is current
    const EGLBoolean makeCurrentResult = eglMakeCurrent(eglDisplay, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT);
    LOG_EGL_RESOURCE("%s:%i:threadid=%i, eglMakeCurrent(eglDisplay=%p, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT)=%i", __FILE__, __LINE__, gettid(), eglDisplay, makeCurrentResult);

    const EGLBoolean eglTerminateResult = eglTerminate(eglDisplay);
    LOG_EGL_RESOURCE("%s:%i:threadid=%i, eglTerminate(display=%p)=%i", __FILE__, __LINE__, gettid(), eglDisplay, eglTerminateResult);
...
static int CreateRenderContext(SvrApplicationContext& appContext)
{
    ...
    EGLint major, minor;
    const EGLBoolean eglInitializeResult = eglInitialize(display, &major, &minor);
    LOGI("EGL version %d.%d", major, minor);
    LOG_EGL_RESOURCE("%s:%i:threadid=%i, eglInitialize(display=%p)=%i", __FILE__, __LINE__, gettid(), display, eglInitializeResult);
    ...
    LOG_EGL_RESOURCE("%s:%i:threadid=%i, eglCreateWindowSurface(display=%p, config=%p, appContext.nativeWindow=%p)=eyeRenderSurface=%p", __FILE__, __LINE__, gettid(), display, config, appContext.nativeWindow, surface);
    ...
    LOG_EGL_RESOURCE("%s:%i:threadid=%i, eglCreateContext(display=%p, config=%p)=eyeRenderContext=%p", __FILE__, __LINE__, gettid(), display, config, context);
...
bool createContext(android_app *pApp)
{
    ...
    if (pApp->window != NULL)
    {
        ...
        LOG_EGL_RESOURCE("%s:%i:threadid=%i, appContext.nativeWindow=%p", __FILE__, __LINE__, gettid(), appContext.nativeWindow);
...
int InputCallback(android_app* pApp, AInputEvent* event)
{
    ...
    if (AInputEvent_getType(event) == AINPUT_EVENT_TYPE_KEY)
    {
        ...
        case AKEY_EVENT_ACTION_DOWN:
            isReserved = pFrmApp->GetInput().KeyDownEvent(iKeyCode);
            break;
        case AKEY_EVENT_ACTION_UP:
            isReserved = pFrmApp->GetInput().KeyUpEvent(iKeyCode);
            break;
...
void EndVR(Svr::SvrApplication* pApp)
{
    if( gSvrInitialized )
    {
        LOGI("SXR_ENDXR");
...
void StartVR(Svr::SvrApplication* pApp)
{
    ...
    if( true || fltWidth >= fltHeight )
    {
        ...
        sxrSetTrackingMode(appContext.trackingMode);
        ...
        LOG_EGL_RESOURCE("%s:%i:threadid=%i, beginParams.nativeWindow=appContext.nativeWindow=%p", __FILE__, __LINE__, gettid(), beginParams.nativeWindow);

        beginParams.optionFlags = 0;
        if (appContext.isProtectedContent)
            beginParams.optionFlags |= kProtectedContent;
        if (appContext.isMotionAwareFrames)
            beginParams.optionFlags |= kMotionAwareFrames;
        if (appContext.isFoveationSubsampled)
            beginParams.optionFlags |= kFoveationSubsampled;
        if (appContext.isCameraLayerEnabled)
            beginParams.optionFlags |= kEnableCameraLayer;

        beginParams.mainThreadId = gettid();
        beginParams.colorSpace =  appContext.isColorSpaceSRGB ? kColorSpaceSRGB : kColorSpaceLinear;

        LOGI("SXR_BEGINXR");
        sxrBeginXr(&beginParams);
...
{
    LOGI("Sxr Application (c) 2015 QUALCOMM Incorporated, Built: %s %s\n\n", __DATE__, __TIME__);
    ...
    if ( sxrInitialize(&initParams) != SXR_ERROR_NONE)
    {
        ...
        //LOGE("android_main: sxrInitialize failed, appication exiting");
        //return;
    }
    ...
    appContext.internalPath = pAppState->activity->internalDataPath;
    appContext.externalPath = pAppState->activity->externalDataPath;
    ...
    while (1)
    {
        ...
        while ((id = ALooper_pollAll((gIsPaused)?-1:0, NULL, &events, (void**)&pSource)) >= 0)
        {
            ...
            if (pAppState->destroyRequested != 0)
            {
                ...
                //{
                //    LOGE("android_main: sxrShutdown failed");
                //}
        ...
        // to handle headless state
        if (pApp->GetHeadlessState())
        {
