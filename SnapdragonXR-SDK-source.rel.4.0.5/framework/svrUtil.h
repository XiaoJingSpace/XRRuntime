...
#define EGL_RESOURCE_LOGGING_ENABLED 0

#define LOGE(...) __android_log_print( ANDROID_LOG_ERROR, "sxr", __VA_ARGS__ )
#define LOGI(...) __android_log_print( ANDROID_LOG_INFO, "sxr", __VA_ARGS__ )
#define LOGW(...) __android_log_print( ANDROID_LOG_WARN, "sxr", __VA_ARGS__ )

#if defined(NDEBUG) || !VERBOSE_LOGGING_ENABLED
#define LOGV(...)
#else
#define LOGV(...) __android_log_print( ANDROID_LOG_VERBOSE, "sxr", __VA_ARGS__ )
#endif

#if EGL_RESOURCE_LOGGING_ENABLED
#define LOG_EGL_RESOURCE(...) __android_log_print( ANDROID_LOG_VERBOSE, "sxr", __VA_ARGS__ )
#else
#define LOG_EGL_RESOURCE(...)
#endif
...
namespace Svr
{
	...
	void SvrGetEyeViewMatrices(const sxrHeadPoseState& poseState, bool bUseHeadModel,
		glm::vec3 ipd_offset_left,	glm::vec3 ipd_offset_right,
		glm::fquat eye_rot_left, glm::fquat eye_eye_left,
		float headHeight, float headDepth,
		glm::mat4& outLeftEyeMatrix, glm::mat4& outRightEyeMatrix);
